/**
 * 当前文件负责：根据世界地图地块位置渲染 Alpha 阶段家园结构。
 */
...
const FALLBACK_TEMP_SHELTER_STAGE_POSITION = TEMP_SHELTER_STAGE_POSITION
const FALLBACK_INCUBATOR_STAGE_POSITION = INCUBATOR_STAGE_POSITION
const FALLBACK_HOME_CONSTRUCTION_STAGE_POSITION = HOME_CONSTRUCTION_STAGE_POSITION
const FALLBACK_GARDEN_STAGE_POSITION = GARDEN_STAGE_POSITION

const TEMP_SHELTER_WIDTH = 135
const TEMP_SHELTER_HEIGHT = 110
const HOME_CONSTRUCTION_WIDTH = 155
const HOME_CONSTRUCTION_HEIGHT = 110
const GARDEN_WIDTH = 120
const GARDEN_HEIGHT = 56
...
export function resolveStageStructureLayout(
  ...
  if (!map) {
    return {
      tempShelter: FALLBACK_TEMP_SHELTER_STAGE_POSITION,
      incubator: FALLBACK_INCUBATOR_STAGE_POSITION,
      homeConstruction: FALLBACK_HOME_CONSTRUCTION_STAGE_POSITION,
      garden: FALLBACK_GARDEN_STAGE_POSITION,
    }
  }
  ...
  const tempShelter = shelterCenter
    ...
    : FALLBACK_TEMP_SHELTER_STAGE_POSITION
  ...
  const garden = gardenCenter
    ...
    : FALLBACK_GARDEN_STAGE_POSITION
...
export function drawTempShelter(layer: Container, position?: StagePoint) {
  const { x, y } = position ?? FALLBACK_TEMP_SHELTER_STAGE_POSITION
...
export function drawHomeConstruction(layer: Container, position?: StagePoint) {
  const { x, y } = position ?? FALLBACK_HOME_CONSTRUCTION_STAGE_POSITION
...
export function drawGarden(layer: Container, position?: StagePoint) {
  const { x, y } = position ?? FALLBACK_GARDEN_STAGE_POSITION
...
function resolveHomeConstructionPosition(input: {
  ...
  return FALLBACK_HOME_CONSTRUCTION_STAGE_POSITION
}
