...
export function ConditionPreview({ channel, color }: ConditionPreviewProps) {
  return (
    <svg viewBox="0 0 256 192" role="img" aria-label={`${channel} 条件图预览`}>
      ...
      {channel === "water" && <path d="M218 0H256V192H224L216 142L226 92L214 48Z" fill={color} />}
      {channel === "road" && <path d="M18 190C54 158 70 140 112 119S162 88 178 54" fill="none" stroke={color} strokeWidth="15" />}
      {channel === "tree" && <><circle cx="32" cy="36" r="20" fill={color} /><circle cx="194" cy="34" r="27" fill={color} /><circle cx="48" cy="151" r="23" fill={color} /></>}
