...
export function generateInitialHomeMapResult(
  ...
  const homeMapStateWithoutEcology: HomeMapState = {
    worldId: input.worldId,
    ownerId: input.ownerId,
    seed,
  ...
  const ecologyState = buildWorldEcologyState({
    homeMapState: homeMapStateWithoutEcology,
    generatedAt: input.now,
  })

  return {
    homeMapState: {
      ...homeMapStateWithoutEcology,
      ecologyState,
      tags: [
        ...homeMapStateWithoutEcology.tags,
        "world_ecology_state_initialized",
        ecologyState.status,
