...
export function buildButlerAutonomyResult(
  ...
  const perception = buildPerception({ input, ecologyState })
...
function buildPerception(input: {
  input: ButlerAutonomyInput
  ecologyState: NonNullable<ButlerAutonomyInput["ecologyState"]>
}): ButlerWorldPerception {
  const resources = input.input.homeMapState.resources
  ...
  const constructionDebt = clampScore(
    100 - Math.min(100, input.input.homeMapState.constructionPlans.length * 25)
  )
  const shelterNeed = clampScore(
    input.input.homeMapState.placements.some((placement) =>
  ...
  return {
    ...
    perceivedFacts: [
      `生态状态 ${input.ecologyState.status}`,
      `地貌 ${input.ecologyState.biomeType}`,
    ...
    opportunities: input.ecologyState.facts
      .filter((fact) => fact.status !== "latent")
      .map((fact) => `${fact.label}:${fact.status}`),
    tags: ["butler_world_perception", input.ecologyState.status, input.ecologyState.biomeType],
  }
}
