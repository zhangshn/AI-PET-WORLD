...
export function buildHomeGoals(home: HomeState): HomeGoalState[] {
  ...
  return sortGoals([
    {
      id: "stabilize_initial_care",
      priority: lifecycle.phase === "initial_care_phase" ? "urgent" : "medium",
      target: "care",
      progress: clamp(
        hasActiveFacility(facilities, "basic_care_station")
          ? 100
          : getFacilityProgress(facilities, "basic_care_station")
      ),
      title: "稳定初始照护区",
      description: "保持初始照护区和基础管理设施稳定。",
      reason: "新世界早期需要先建立基础照护与管理秩序。",
      recommendedBehaviorKinds: ["home_maintenance", "space_tidying"],
      tags: ["home_goal", "initial_care", `phase_${lifecycle.phase}`],
    },
    {
      id: "build_temporary_shelter",
      priority: priorityFromPhase(lifecycle.phase, "temporary_shelter_phase"),
      target: "space",
    ...
    },
    {
      ...
      priority: lifecycle.phase === "basic_living_phase" ? "high" : "medium",
      target: "facility",
      progress: clamp(
        (
    ...
    },
    {
      ...
      priority: lifecycle.phase === "stable_home_phase" ? "medium" : "low",
      target: "future_world",
      progress: clamp(
