...
文档版本：`AI-CONSOLE-ARCHITECTURE-1.7`
...
V23训练历史观察按数据API合同第3.2.4节增加独立后台发现/派生索引/版本适配层。服务端Node启动生命周期负责单实例有界更新；查询GET不写索引、不启动进程或扫描。历史发现可读取固定存储catalog和登记热/冷根中的训练证据，不能回填当前执行身份、新平台任务、资格或遥测。原当前登记来源的断链仍失败关闭，但不阻断独立可核验历史来源；逐来源覆盖与逐制品缺口必须保留。此例外不改变下述主登记、控制、当前执行、审核和发布层的隔离规则。
