...
程序符合状态：`v22_history_connected_legacy_detail_and_coverage_partial`
...
9. `observability/current-execution`是当前执行身份的唯一HTTP入口，不得扫描历史目录补值。历史查询按第9.2节及数据API合同第3.2.4节读取独立观察索引和受验证历史事务；不得创建第二份当前身份。两者均不得调用旧页面API，历史GET不得触发后台扫描。

### 9.2 V23真实训练历史、制品与动态查看

本节将AP-03历史与Checkpoint、AP-09训练归档接入真实机器证据；适用接口以数据/API合同第3.2.3至3.2.4节为准。它是历史观察接入，不新增训练、审核、发布或原始证据迁移权力。

1. 历史观察范围包含受验证登记历史、既有存储catalog发现线索、归档清单、运行清单和生成结果，使用数据API合同第3.2.4节的独立后台增量索引与逐来源覆盖。登记断链不得隐藏其他独立可核验历史。旧页面和旧API仍禁用；不得按目录名称、修改时间或GPU进程猜测历史或当前身份，不补写初始化登记来制造记录。发现记录不等于核验通过、活动任务或正式资格。
2. 历史列表、选中Run、当前活动执行和最近训练终态分别呈现。按Run展示训练/实验类别、分辨率、数据样本、原始时间、真实优化步、指标、Checkpoint、终态和证据可用性；实验完成不能显示成视觉合格、正式Stage通过或已发布。
3. 原图、条件、生成输出及Checkpoint只能沿所选Run的受验证请求/结果绑定读取。界面提供原图与输出对照、样本/实验臂/种子标识和可追溯指标；二进制权重只展示元数据，不加载执行。未知、未记录、绑定冲突和未核验字节分别标识，不用默认值补齐。
4. 历史与Checkpoint页面自动查询，正常目标间隔2秒，单次请求有超时且禁止重叠；失败显示断线/过期及最近成功时间，保留旧值时必须标记过期。视图切换取消旧请求，恢复连接后自动刷新，所选Run不能跳到另一条记录。当前态1秒及资源250毫秒单例轮询保持原合同，不把目标间隔宣传为每个探针的实际完成速度。
5. 分页必须固定已核验修订和稳定身份；展示登记覆盖范围、尚未接入或缺失证据，不把有限的已登记历史宣称为磁盘全部历史。新增已提交记录无需修改代码即可进入列表；目录中未登记文件不自动获得可信资格。
6. GET无写入、训练启动、服务控制或证据迁移；刷新列表不反复读取大型Checkpoint或全部图像。图片按需读取并核验，源绑定或字节改变不能命中旧内容缓存；读取并发、字节和历史遍历均需明确限额及失败状态。
7. 验收必须覆盖真实历史展示、原图/输出/指标对应、Checkpoint绑定、自动刷新、断线恢复、分页和选择保持、越界与篡改拒绝，以及AP-01/AP-03/AP-04当前身份和全局资源观察回归。合成新增记录只在隔离测试使用，不写生产证据。部署前保留本次差异的可回退版本，回退只撤销代码，不删除训练证据或回滚业务状态。
8. 选择历史记录后必须有明确可见的只读详情入口与加载/失败状态，不能只变更选中颜色。详情展示该Run的已核验登记事件、原始事件时间和证据入口；登记时间与训练开始/完成时间分开标注。历史证据Schema尚未适配时，用中文说明缺失内容并保留可读取的终态/事件证据，机器错误码仅作为辅助诊断信息。登记事件不冒充逐步训练日志，也不与AP-07控制命令事件混用；时间线完整性受同一历史覆盖边界约束。
