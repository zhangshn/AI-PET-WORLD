...
更新时间：2026-08-09 16:43:24 +08:00
...
| 2 | V7失败学习与R5隔离候选 | 由本地自研AI程序读取训练和机器审核失败证据，生成有界修复提案、训练器合同、隔离配置和不可变终态；旧Checkpoint、审核阈值及R2/R3/R4来源证据保持不可变 | 固定五阶段中的第1、2、3阶段已完成，固定总进度为3/5（60%）。第4阶段第一次正式Stage 0训练及其后V7/V8/V9候选Smoke均已失败关闭。V9已证明四类对象独立梯度、west道路读出和精确17项Manifest导出可运行，也完成了真实30 Epoch有界学习；但五张固定预览机器审核0/5通过，四类对象视觉语义均未达到条件一致性，因此V9 Smoke Checkpoint不得晋级，Stage 1/2未启动，第4阶段仍未完成，第5阶段严格复验未启动 | R4/R5来源证据保持不可变；失败预览不得作为训练目标；审核阈值不得降低；道路边界必须绑定当前执行样本，不得跨样本继承。第4阶段正式训练固定使用64/64合格数据与48/8/4/4划分，Stage 0/1/2按256×192、512×384、1024×768顺序执行，每阶段40 Epoch及Epoch 1/5/10/20/30/40固定预览机器审核；Stage 0从固定随机初始化开始，Stage 1/2只能加载本次前一Stage的Checkpoint。任一预检、训练或机器审核失败立即停止，后续Stage不得启动且不自动重试 | V9只读归因已正式裁决为重复既有视觉失败，当前V9候选路线退出，不得继续参数修复或重跑同类Smoke。下一合法动作必须由Owner在“选择实质不同的模型路线”与“停止Stage4候选开发”之间作出明确业务决策；作出新路线决策前没有完整训练准入。正式推理、Checkpoint正式晋级、RuntimeFrame和进入世界继续禁止 |
...
| 4 | Stage 0→1→2完整训练 | 使用64/64合格数据和48/8/4/4划分，依次完成256×192、512×384、1024×768三阶段训练及固定预览机器审核 | 进行中，尚未完成 |
...
| 18 | Stage4完成条件 | 新的完整训练必须从Stage 0固定随机初始化开始，Stage 1/2只继承本次前一Stage；三阶段训练和机器审核全部通过后，固定总进度才从3/5（60%）更新为4/5（80%） |
...
## AI Painter R5 Stage4 当前准入状态（2026-08-09 17:57 +08:00）
...
- 当前没有Stage4完整训练准入。下一合法动作必须由Owner明确选择：建立一条与V8/V9实质不同的模型路线，或停止Stage4候选开发；在该业务决策形成前，不再执行参数级修补或GPU Smoke。
- 正式推理、Checkpoint晋级、RuntimeFrame和进入世界仍未授权。只有新的Stage 0→1→2完整训练及各阶段机器审核全部通过后，固定总进度才能从3/5（60%）更新为4/5（80%）。
