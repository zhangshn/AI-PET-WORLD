...
更新时间：2026-09-23 01:22:17 +08:00

状态：active-long-term-data-and-provenance-contract

文档版本：`AI-PAINTER-DATA-PROVENANCE-1.7`

生效日期：`2026-09-23`

替代版本：`AI-PAINTER-DATA-PROVENANCE-1.6`
...
本节只澄清第8节与第9.1节在单世界先行MVP中的来源隔离语义，不降低48／8／4／4用途隔离、challenge未见、来源许可、样本身份、WorldFacts／条件／RGB一致性或GPU训练门槛。

- **必须同组**：同一RGB或其裁切／缩放／修图派生；同一监督母版、标签或样本特定几何；生成一个样本时读取另一样本的RGB、条件、结构实例、人工修复结果或评估反馈；以及已复现的邻区联合几何、边界水流配对等样本特定生成上下文。
- **继续跨组阻断**：不同split的主要样本来源窗口存在正面积重叠；图片、条件内容、world、主题架构或实例身份重复；当前模型谱系把非train内容用于权重更新，或把challenge内容／指标用于训练、调参和Checkpoint选择。
- **不自动构成同组**：样本只因来自同一公开数据提供方、同一泰国区域、边界相接，或在拆分前由固定且不读取任何样本RGB、监督目标、人工修复或评估反馈的公共区域预处理，共享DEM、土地覆盖等外生测量输入。此类输入必须记录数据身份、固定算法和实际用途；它只证明公共协变量共享，不证明目标泄漏，也不证明样本已合格。
- **证据不足**：无法证明预处理在拆分前冻结、无法排除样本目标或评估反馈进入计算、或依赖实际承担样本特定生成上下文时，状态必须为`indeterminate_due_to_missing_evidence`并阻断资格；不得默认安全，也不得把全部未知关系合并成一个大组。
- **验收目标**：当前challenge验证的是未参与当前适用模型训练和选择的结构／条件到RGB映射，不声明跨国家、跨生态区或完全独立公共地理底图的泛化能力。进入多区域扩展时必须建立新的地理外推数据版本和验收合同。

正式机器分类合同为`data/ai-painter/system-governance/ai-painter-mvp-source-isolation-classification-contract-v1.json`。分类器只能裁决已绑定的关系记录；它不能从缺少记录推导合格，也不能直接签发数据、模型或GPU资格。
...
本节只对单一原始自然世界的先行MVP生效，不取消第9节及MAP-023／MAP-024的长期要求，不扩展至人物、建筑、多区域或批量世界交付。

- **隔离项**：与全部历史成功、待审、失败及否决地图进行结构／实例细节创作唯一性比较。该项在本阶段标记为`deferred_out_of_current_mvp_scope`，不作为本阶段训练、候选或发布的阻断条件；不得标为`passed`，不得宣称已经完成全历史唯一性验证。
- **保留项**：当前64份数据内部的结构／内容去重与独立容量、48／8／4／4用途、来源许可、派生及上下文依赖、空间来源隔离、适用模型谱系的训练／选择历史、challenge未见与regression用途限制，以及WorldFacts／条件／RGB一致性。历史资料若用于判定泄漏或权属，仍须读取核验，不能因同时参与创作唯一性检查而全部跳过。
- **保留资产**：原唯一性规则、程序、合同及成功／失败证据原样保留，不移入退役目录，不删除，不覆盖旧裁决。只调整新作用域的要求适用性；旧候选仍保留原资格与失败状态。
- **作用域绑定**：新数据、能力及执行身份须共同绑定本节文档版本、隔离要求ID、仍生效要求和恢复条件。身份缺失、范围不符或同时含泄漏风险的发现不得静默过滤；混合发现须分别给出创作唯一性与数据隔离结论，不能通过删除整个历史审核结果放行。
- **恢复条件**：在进入多区域扩展、批量世界生成，或任何声明全历史结构／细节唯一性的后续能力前，恢复第9节完整检查并取得真实证据。恢复时复核历史资产和适用实现；本阶段产物纳入后续比较库但不得自动取得完整资格，未通过则阻断后续扩展，不追改本阶段原始记录。
- **程序边界**：当前为`program_adoption_pending`。必须先完成版本化适用性处理及正反测试，证明本阶段只隔离创作唯一性、泄漏等保留项仍会阻断、超范围自动恢复完整要求，然后才能由本地程序签发新作用域结论。文档更新不产生训练许可，也不把旧仅阻断预检改写为成功入口。

实现责任：[`ai-painter-mvp-requirement-scope.mjs`](../../scripts/lib/ai-painter-mvp-requirement-scope.mjs)承担保守分类；[`inspect-ai-painter-mvp-requirement-scope.mjs`](../../scripts/inspect-ai-painter-mvp-requirement-scope.mjs)只读取显式绑定的历史诊断及其输入字节，生成独立作用域评估，不重新扫描历史或更新生产登记。该评估不替代新数据／能力／执行绑定，也不能直接用于训练放行；生产接入须消费合格的新作用域身份并重新核验保留要求。
