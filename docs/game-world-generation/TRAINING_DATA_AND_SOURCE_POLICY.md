...
文档版本：`AI-PAINTER-DATA-PROVENANCE-1.0`
...
程序不得从编号、队列、旧执行包、页面状态或聊天中的“允许”“继续”推导新的能力版本。同一条件已有待审、通过或失败历史时，重试必须符合当前能力版本的次数和停损合同；真实失败路线重启必须进入新的变更决定。
