...
本文档只记录当前进度。项目总入口看 [README](../README.md)，业务定义看 [业务说明书](./BUSINESS_SPEC.md)，执行顺序看 [唯一执行计划表](./EXECUTION_PLAN.md)。

## 1. 当前阶段

```txt
当前主线：自然家园小模型训练 + 自然家园 VisualJudge + 第一张自然家园 ApprovedFrame
当前阶段：阶段 A 文档体系已完成，下一步回到阶段 C 自然家园 VisualJudge 强化
当前最新结果：业务说明书、架构表、目录结构表、独立进度表已补齐；V91 自然家园候选完成 VJ-1 审核，17 / 17 通过 VJ-1
...
| 文档入口整理 | 完成 | 100% | README、业务说明书、执行计划、架构、目录、进度表已分离 |
| 业务说明书 | 完成 | 100% | `docs/BUSINESS_SPEC.md` 已补齐玩家流程、管家灵魂、世界自主、视觉边界 |
| 架构总表 | 完成 | 100% | `docs/ARCHITECTURE.md` 已补齐架构层、数据流、模块边界 |
| 目录结构表 | 完成 | 100% | `docs/DIRECTORY_STRUCTURE.md` 已补齐根目录、docs、页面、Runtime、AI Painter、数据目录职责 |
