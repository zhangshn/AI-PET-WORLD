...
文档版本：`DOCUMENT-AUTHORITY-1.8`

生效日期：`2026-09-21`

替代版本：`DOCUMENT-AUTHORITY-1.7`
...
分段交付及前期最小范围以业务规格第3.2节为准：先原始自然世界，再人格映射及角色行动，房屋与自主建设后置；完整产品验收不得倒置为每个先行里程碑的全部前置条件。该范围澄清不改变现行机器合同或既有失败证据，程序符合状态必须另行验证。
