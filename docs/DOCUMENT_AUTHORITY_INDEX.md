...
Owner当前明确命令可以批准新的研发任务或业务选择，但聊天本身不是持久能力发布记录。新模型、Loss、数据、训练、阈值或能力版本发布必须形成不可变能力变更与发布身份；已发布能力版本内的生成、固定验证、机器审核、RuntimeFrame发布或回退、世界运行和记录由本地系统按受信发布合同自主完成。`AGENTS.md`、业务文档、架构文档和机器合同不得把这两类路径重新合并为逐任务人工授权。
...
历史合同保留原始字节和SHA用于复核历史运行，并由`data/ai-painter/system-governance/contract-supersession-index-v1.json`登记后继合同与停用状态。当前能力发布只从`data/ai-painter/system-governance/ai-painter-capability-release-registry-v1.json`读取；该注册表当前没有正式AI Painter能力发布记录。
