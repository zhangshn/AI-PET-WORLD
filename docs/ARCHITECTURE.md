...
更新时间：2026-08-24 07:05:44 +08:00

状态：long-term-architecture-reference

文档版本：`AI-PET-WORLD-ARCHITECTURE-1.0`
...
能力发布身份不能由调用方布尔字段或外部口头声明成立。运行器必须从受信发布注册表定位不可变能力发布文件，重新计算发布文件、Owner能力发布决定、数据版本、模型产物、审核合同、Runtime接口合同和23通道条件合同的SHA-256，并验证发布状态与绑定身份；只有完整验证通过后才能签发一次性内部任务票据。消费票据时必须再次计算票据SHA-256并重新核对当前受信发布记录。当前注册表没有正式AI Painter能力发布记录，因此正式推理、RuntimeFrame和进入世界保持不可用。
