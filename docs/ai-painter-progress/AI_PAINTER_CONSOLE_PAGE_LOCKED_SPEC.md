...
状态：active-lock / 控制台GET只读与实时状态已实现 / 原图项目所有者审核POST命令入口已授权
...
7. 项目所有者于2026-07-23授权的完整地图原图“通过 / 拒绝”按钮是唯一写操作例外：GET页面继续严格只读；按钮通过本机POST提交明确owner命令，所有业务文件、SQLite索引、审核历史、失败学习、容量贡献和数据包均由正式程序自动写入。页面不得直接代写任何业务状态。
8. 审核按钮不能启动图片生成、下一槽位准备、GPU训练、推理、Runtime晋级或`/world`发布；这些动作仍分别受正式文档与项目所有者单独授权控制。
