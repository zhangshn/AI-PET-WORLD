...
| 完整地图三级 | `/ai-painter-progress/original-images/complete-maps/types/<typeId>` | 只展示一个固定完整地图原图类型的时间下拉框和当前选中记录 |
| 记录详情 | `/ai-painter-progress/original-images/<categoryId>/<recordId>` | 展示单条原图、原始路径、来源、状态、绑定和审核信息；完整地图记录从类型页进入 |

页面是只读资料库。GET 页面不得创建目录、复制图片、生成清单、修改时间或补造训练记录。

页面固定为分类信息架构，文件目录可以具有更多语义深度。`complete-maps` 二级页固定只显示四个类型入口，类型页一次只展开一条程序记录；其他分类仍从二级页直接查看记录。植物分类必须展示 `plantKind -> speciesId -> lifeStage -> season -> recordId` 及当前现实物种目录；记录详情页仍只对应单条真实记录。URL 不机械复制每一个文件夹层级，但页面显示的相对路径和目录顺序必须与接收程序完全一致。

所有原图从现在开始必须在控制台展示，不是后期功能。接收程序成功保存原图后必须原子更新 `index.json`；控制台通过轻量事件流监听索引和生成失败证据变化并自动刷新，不需要项目所有者或 Codex 手工刷新、归档或补写。`complete-maps` 类型页必须通过时间下拉框保留该类型全部真实记录，选择后只展示对应一条记录的图片、状态、尺寸和路径；记录详情页展示完整图片与全部来源、权属、分类和审核数据。事件流不得循环扫描训练目录或发送完整训练台账。
...
### 4.2 `complete-maps` 固定类型

| typeId | 页面名称 | 自动归类规则 |
|---|---|---|
| `autonomous-generation-training-originals` | 自主生成训练原图 | 项目所有者审核通过并具有 `autonomousGenerationTrainingOriginal.sequenceNumber` 的记录 |
...
四个类型互斥。分类只根据程序保存的记录字段计算，不移动、不复制、不重命名 `complete-maps/<recordId>/` 真实目录。失败类型必须同时显示有图拒绝记录与无图生成失败；无图失败必须显示失败码、路线、UTC、北京时间和不可变证据路径。

项目所有者于 2026-07-16 明确通过第005号 V2。程序已将其写为 `自主生成训练原图第001张`，记录 ID 固定为 `ai-cold-start-condition-pair-005-seasonal-evergreen-semi-evergreen-forest-v2`，序号为 `1`。它是自主生成训练原图新序列的第一张，不等于 RuntimeFrame，也不得直接进入 `/world`。
