...
更新时间：2026-08-04 11:27:27 +08:00
...
-> 程序保存的 config / dataset manifest / source-index / stage manifest /
   failure / owner-action-request / ledger + nvidia-smi + Windows只读硬件查询
-> 统一只读快照
```
...
1. 固定顶部训练任务总览：是否正在训练、当前任务/训练链/Run、模型、Dataset、Stage总进度、当前Epoch、目标Epoch、父子Checkpoint、资源、活动阻断和有效Owner动作请求。即使当前没有训练，也必须明确显示“未在训练”及最新训练终态，不得只显示模糊失败句。
...
8. 当前训练链程序事件和不可变证据路径。
9. 参数说明中心是本地自研AI模型的统一数据字典，不是页面私有提示词集合。字典必须机器可读，并覆盖模型身份、数据与条件、训练过程、Checkpoint与产物、严格复验、机器审核、Token与计算量、硬件与运行环境、Owner授权与治理；每项至少保存中文名、英文代码、类别、字段类型、通俗说明、专业解释、阅读规则和权威数据源，并支持中文、英文代码、拒绝码和相关词模糊查询。Epoch表头可直接打开对应说明。
...
严格复验入口只承担证据读取和人工观察，不得在卡片中代写Owner通过、重跑复验、启动训练或修改审核文件。复验图由既有只读图片接口按项目内允许路径提供；批次选择、图片查看和关闭弹窗不得改变URL或顶部训练任务上下文。
...
同一模型链存在多份证据时，必须先核对`modelId`、`datasetPackageId`、`checkpointSha256`和训练链/验证批次身份；任务不匹配的证据不得参与当前状态竞争。身份匹配后按证据时间选择更新事实，只有时间相同时才按终态优先级裁决；陈旧心跳不得覆盖更新的失败、阻断或等待授权终态。旧证据不得删除或改写。资源门禁、Owner暂停、授权未消费、执行中、执行失败、验证失败和完成等待独立授权必须使用不同状态码。缺少最新证据时显示`unknown_or_stale`，不得回退成旧结论或由页面猜测。

训练、验证、复核与失败归因程序必须在自身终态路径内自动写入不可变报告、最新状态指针、程序事件、SQLite索引、Token/资源账本和Owner动作请求。控制台GET接口保持只读；Codex不得作为运行时写入器、状态数据库或闭环依赖。
