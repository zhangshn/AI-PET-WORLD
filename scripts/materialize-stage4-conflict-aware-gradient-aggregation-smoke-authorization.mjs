...
if (!/^[0-9]{8}-[0-9]{9}$/.test(integrationRunId ?? "") || !/^[0-9]{8}-[0-9]{9}$/.test(executionRunId ?? "")) {
  throw new Error("integration and execution runIds are required")
}
...
const integration = `.runtime/ai-painter/stage4-conflict-aware-gradient-aggregation-smoke-entry-integrations/${integrationRunId}`
...
const implementationAuthorization = `${integration}/resolved-implementation-authorization.json`
const implementationConsumption = `${integration}/resolved-implementation-consumption.json`
for (const file of [inactiveConfig, registryPath, cpuReport, implementationAuthorization, implementationConsumption]) {
  if (!fs.existsSync(resolve(file))) throw new Error(`bound source missing:${file}`)
}
...
const role = (name) => {
  ...
  if (
    registry.status !== "stage4_execution_evidence_eligibility_registered"
    || entry?.disposition !== "active_reusable_success_evidence"
  ) throw new Error(`canonical role invalid:${name}`)
  return bind(entry.canonicalPath)
}
const requestId = `owner-authorized-stage4-conflict-aware-existing-gradient-aggregation-30-epoch-model-smoke-${executionRunId}`
...
const executionRoot = `.runtime/ai-painter/stage4-conflict-aware-gradient-aggregation-smokes/${executionRunId}`
writeExclusive(authorizationPath, {
  ...
  taskIdentity: {
    ...
    trainingObjectiveContractId: "stage4_conflict_aware_existing_gradient_aggregation_v1",
...
console.log(JSON.stringify({
  status: "stage4_conflict_aware_gradient_aggregation_smoke_authorization_materialized_unconsumed",
