...
const TASK_ID = "compile_native_responsibility_residual_controlled_smoke_contract"
const RUN_ID = `stage4-native-responsibility-residual-smoke-${compactUtc()}-01`
...
const INACTIVE = resolve(".runtime/ai-painter/stage4-native-responsibility-residual-cpu-support/stage4-native-responsibility-residual-cpu-20260827232250-01/inactive-config.json")
const GPU_TERMINAL = resolve(".runtime/ai-painter/stage4-native-responsibility-residual-readonly-gpu/stage4-native-responsibility-residual-gpu-20260827232754-01/phase-terminal.json")
...
const CPU_CHECKER = resolve("ml/ai-painter/scripts/check_stage4_native_responsibility_residual_smoke_cpu.py")
const COMPILER = resolve("ml/ai-painter/scripts/compile_stage4_native_responsibility_residual_smoke_active_config.py")
const CONTRACT_ROOT = resolve(`.runtime/ai-painter/stage4-native-responsibility-residual-smoke-contracts/${RUN_ID}`)
const CONTRACT = path.join(CONTRACT_ROOT, "controlled-smoke-contract.json")
const EXECUTION_ROOT = resolve(`.runtime/ai-painter/stage4-native-responsibility-residual-controlled-smokes/${RUN_ID}`)
...
assert.equal(read(CURRENT_GPU_TERMINAL).status, "native_responsibility_residual_readonly_gpu_qualification_succeeded")
...
verifyFile(GPU_TERMINAL, "a1652ec3ec19181d6928c759fb19f8fc2faabb9f56273cd6b0510d28060120f5", "qualified readonly GPU terminal")
...
const sourceEvidence = [
  ...
  ["structureContract", resolve("ml/ai-painter/scripts/ai_painter_native_responsibility_residual_contract.py")],
...
assert.equal(smokeCpu.status, "stage4_native_responsibility_residual_smoke_cpu_gate_passed")
writeExclusive(path.join(CONTRACT_ROOT, "cpu-report.json"), { schemaVersion: "stage4-native-responsibility-residual-smoke-contract-cpu-report-v1", status: "passed", ...smokeCpu, contract: bind(CONTRACT), sourceEvidence, ownerAuthorizationRequired: false, recordedAtUtc: new Date().toISOString() })
writeExclusive(path.join(CONTRACT_ROOT, "evidence-isolation-audit.json"), { schemaVersion: "stage4-native-responsibility-residual-smoke-evidence-isolation-audit-v1", status: "passed", currentCapabilityOnly: true, historicalSmokeRead: false, historicalCheckpointRead: false, crossRunOutputReuse: false, sourceEvidence, recordedAtUtc: new Date().toISOString() })
...
writeExclusive(paths.trainerPreflightBinding, { schemaVersion: "stage4-native-responsibility-residual-execution-preflight-binding-v1", status: "verified", contract: bind(CONTRACT), readonlyGpuTerminal: bind(GPU_TERMINAL), trainerPreflight, trainingOutputCreated: false, gpuStarted: false, optimizerCreated: false, backwardExecuted: false, trainingStarted: false, recordedAtUtc: new Date().toISOString() })
if (!resources.passed) await closeInfrastructureFailure("resource_preflight_failed", resources.blockers.join(","), false)

writeExclusive(paths.ticket, { schemaVersion: "ai-painter-local-internal-capability-ticket-v1", status: "issued_not_consumed", ticketId: `local-ai-${RUN_ID}`, capabilityVersion: CAPABILITY, runId: RUN_ID, scope: "one_native_responsibility_residual_30_epoch_controlled_smoke_closed_loop", parentContract: bind(CONTRACT), capabilityAuthority: "local_ai_pet_world_program", singleUse: true, persistedReplayProtection: true, cannotExpandParentContract: true, ownerAuthorizationRequired: false, issuedAtUtc: new Date().toISOString() })
...
advanceCapabilityLifecycle({ root: ROOT, capabilityVersion: CAPABILITY, targetState: qualification.qualified ? "controlled_smoke_completed" : "rejected", evidence: { schemaVersion: "ai-painter-capability-stage-evidence-v1", capabilityVersion: CAPABILITY, targetState: qualification.qualified ? "controlled_smoke_completed" : "rejected", status: qualification.qualified ? "passed" : "failed", bindings: [paths.terminal, paths.finalization, paths.manifest, paths.machineReview, paths.qualification].map(bind), ownerAuthorizationRequired: false } })
const advanced = await advanceCurrentExecutionRegistry({ projectRoot: ROOT, capabilityVersion: CAPABILITY, packageId: RUN_ID, taskId: nextLegalAction, taskKind: qualification.qualified ? "formal_stage_training" : "project_level_route_decision", runId: RUN_ID, lifecycleStage: qualification.qualified ? "controlled_smoke_completed" : "rejected", executionState: "package_materialized", activity: "planned_not_started", taskCapsulePath: projectPath(paths.capsule), terminalEvidencePath: projectPath(paths.terminal), latestTrainingTerminal: { runId: RUN_ID, path: projectPath(paths.terminal), sha256: sha256(paths.terminal), status: terminalStatus, evidence: { executionState: bind(paths.executionState), machineReview: bind(paths.machineReview), reviewProgress: bind(paths.reviewProgress), trainingProgress: bind(paths.progress) } } })
appendAiPainterProgramEvent({ id: `stage4-native-responsibility-residual-controlled-smoke-${RUN_ID}`, timestamp: new Date().toISOString(), action: "stage4_native_responsibility_residual_controlled_smoke_closed_loop", runId: RUN_ID, kind: "controlled_smoke", status: qualification.qualified ? "success" : "failed_closed", title: "Stage4 native responsibility residual 30 Epoch controlled Smoke completed", titleZh: qualification.qualified ? "Stage4原生条件编码责任残差30 Epoch受控Smoke通过" : "Stage4原生条件编码责任残差30 Epoch受控Smoke真实视觉失败并关闭", detailZh: `${EPOCH_COUNT} Epoch训练完成；机器审核${review.previewPassCount}/${review.previewCount}，资格=${qualification.qualified}。`, evidencePath: projectPath(paths.terminal), evidenceSha256: sha256(paths.terminal), fixedTotalProgress: progressFixed() })
...
async function reviewPreviews() {
  ...
  for (const artifact of manifest.fixedPreviews) {
    ...
    const normalized = await normalizePreviewWithWindowsSafeIo({ sourcePath, finalAssetPath: normalizedPath, workRoot: resolve(".runtime/ai-painter/native-responsibility-residual-review-work"), workId: shaText(RUN_ID).slice(0, 16), epoch })
    const [aesthetic, alignment] = await Promise.all([
      auditAiAssistedProfessionalAesthetic(normalized.shortOutputPath),
      auditAiAssistedConditionAlignment({ record: { recordId: `native-responsibility-residual-smoke-${epoch}`, conditionBinding: { conditionPackPath: sample.conditionPackPath, worldId: conditionPack.worldId, tick: conditionPack.tick }, classification: sample.classification }, imagePath: normalized.shortOutputPath, referenceImagePath: resolve(sample.imagePath) }),
    ])
