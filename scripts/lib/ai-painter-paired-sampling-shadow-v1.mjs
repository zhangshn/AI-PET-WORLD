// Paired sampling grids through the EXISTING six-phase runner.
...
export const ADAPTER = "scripts/lib/ai-painter-paired-sampling-shadow-v1.mjs";
export const WORKER = "ml/ai-painter/scripts/compare_paired_sampling_grids.py";
export const NODE_TEST = "scripts/tests/test-ai-painter-paired-sampling-shadow.mjs";
export const PY_TEST = "ml/ai-painter/tests/test_paired_sampling_grids.py";
...
export function requestOf(context) {
  ...
  assert.equal(request.schemaVersion, "ai-painter-paired-sampling-shadow-request-v1");
  assert.equal(request.identity, context.packageIdentity);
  assert.equal(request.outputRoot, context.outputRoot);
  assert.equal(request.mode, "cpu_paired_sampling_grids_no_training_or_release");
...
export async function preparePairedSamplingShadowPackage(root = process.cwd()) {
  ...
  assert.equal(previous.schemaVersion, "ai-painter-periodic-layer-shadow-result-v1", "current task is not the prerequisite layer diagnosis");
  assert.equal(previous.executionState, "completed");
  assert.equal(previous.status, "periodic_layer_shadow_completed_not_visual_qualified");
  ...
  const payload = { schemaVersion: "ai-painter-paired-sampling-shadow-request-v1",
    mode: "cpu_paired_sampling_grids_no_training_or_release", sourceCurrent, sourceResult, sourcePackage,
  ...
  const identity = "paired-sampling-shadow-" + sha(bytes(payload));
...
async function register(context, request, evidence, begin) {
  ...
  const capsule = persist(root, `${context.outputRoot}/${begin ? "begin" : "finish"}-capsule.json`, {
    ...
    evidence: [bind(root, `${context.outputRoot}/diagnostic-request.json`), evidence].map((b, i) => ({ ...b, kind: `paired_sampling_shadow_${i}`, sha256Verified: true })) });
  const prepared = await prepareCurrentExecutionRegistryAdvance({ projectRoot: root, capabilityVersion: request.identity,
    packageId: request.identity, taskId: request.identity, runId: request.identity, taskKind: "cpu_paired_sampling_shadow",
    taskGoal: "Compare 50, 25 and 10 complete sampling grids on both current frozen checkpoints; no selection or training",
    queueStatus: begin ? "running" : value.executionState, nextMachineAction: null, lifecycleStage: "isolated_implementation",
    executionState: begin ? "executing" : value.executionState, activity: begin ? "paired_sampling_shadow_running" : value.status,
...
async function phase(context, operation) {
  ...
  try {
    ...
    const artifact = persist(context.projectRoot, `${context.outputRoot}/failure-${context.phase}.json`, {
      status: "paired_sampling_shadow_failed_closed", executionState: "failed_closed", runId: context.packageIdentity,
    ...
    return { status: "failed", failureKind: "evidence", failureCode: "paired_sampling_shadow_check_failed", artifact };
...
export const preflight = context => phase(context, async request => {
  ...
  const artifact = persist(context.projectRoot, phaseFile(context, "preflight"), { status: "paired_sampling_shadow_preflight_passed",
...
function finite(v){assert(typeof v==="number"&&Number.isFinite(v)&&v>=0,"invalid nonnegative metric");return v;}
export function summarizePairedSampling(source,replay,outputRoot) {
  summarizeTrainingComparison(source,replay.baselineReplay);
  assert.equal(replay.schemaVersion,"ai-painter-paired-sampling-comparison-v1");
  assert.equal(replay.status,"sampling_comparison_completed_not_qualified");
  for(const [k,v] of Object.entries({fixedSeedRollouts:36,imagesWritten:24,checkpointsWritten:0,
    optimizerSteps:0,cudaInitialized:false,modelAndNormalizationUnchanged:true,sourcePngPixelsReproduced:12}))assert.equal(replay[k],v);
  assert.equal(replay.rows.length,36);assert.equal(replay.images.length,24);
  const expected=source.rows.flatMap(r=>STEPS.map(steps=>[r.arm,r.sampleId,r.seed,steps]));
  assert.deepEqual(replay.rows.map(r=>[r.arm,r.sampleId,r.seed,r.steps]),expected);
  const imagePaths=new Set();
  for(let i=0;i<36;i++){
    const row=replay.rows[i],baseline=source.rows[Math.floor(i/3)];
    assert.equal(row.split,"train");assert.equal(row.targetUsedForInitialization,false);assert.equal(row.fullEndpoint,true);
    assert(/^[a-f0-9]{64}$/.test(row.noiseStateSha256));
    assert.equal(row.noiseStateSha256,replay.rows[i-i%3].noiseStateSha256);
    if(i>=18)assert.equal(row.noiseStateSha256,replay.rows[i-18].noiseStateSha256);
    for(const metric of METRICS)finite(row.measurements.final[metric]);
    for(const name of HEADS){
      finite(row.measurements.semanticRegions[name].rgbMae);
      assert.equal(row.measurements.semanticRegions[name].pixelWeight,baseline.measurements.semanticRegions[name].pixelWeight);
    }
    assert.equal(row.measurements.outsideCoverageMaxAbsoluteChange,0);
    assert.equal(row.measurements.coverageFraction,baseline.measurements.coverageFraction);
    const sampleIndex=Math.floor((i%18)/9),seedIndex=Math.floor((i%9)/3);
    if(row.steps===50){
      assert.equal(row.baselinePixelsAndMeasurementsExact,true);
      assert.deepEqual(row.measurements,baseline.measurements);
      const expectedPath=path.posix.dirname(replay.sourceResult.path)+"/"+row.arm+"-"+sampleIndex+"-"+seedIndex+".png";
      assert.equal(row.image.path,expectedPath);
      assert(source.artifacts.some(b=>b.path===row.image.path&&b.sha256===row.image.sha256));
    }else{
      assert.equal(row.baselinePixelsAndMeasurementsExact,false);
      assert.equal(row.image.path,outputRoot+"/"+row.arm+"-"+row.steps+"-"+sampleIndex+"-"+seedIndex+".png");
      assert(/^[a-f0-9]{64}$/.test(row.image.sha256));
      assert(replay.images.some(b=>b.path===row.image.path&&b.sha256===row.image.sha256));
      assert(!imagePaths.has(row.image.path));imagePaths.add(row.image.path);
    }
  }
  assert.equal(imagePaths.size,24);assert.equal(new Set(replay.images.map(b=>b.path)).size,24);
  const groups={};
  for(const arm of ARMS){
    groups[arm]={};
    const rows=replay.rows.filter(r=>r.arm===arm);assert.equal(rows.length,18);
    for(const steps of [25,10]){
      const stats={};
      for(const metric of [...METRICS,...HEADS]){
        const value=r=>METRICS.includes(metric)?r.measurements.final[metric]:r.measurements.semanticRegions[metric].rgbMae;
        const pairs=Array.from({length:6},(_,i)=>[value(rows[i*3]),value(rows[i*3+STEPS.indexOf(steps)])]);
        const changes=pairs.map(([a,b])=>b-a),pct=pairs.every(([a])=>a>0)?pairs.map(([a,b])=>(a-b)/a*100):null;
        stats[metric]={improvedCount:changes.filter(v=>v<0).length,worseCount:changes.filter(v=>v>0).length,
          equalCount:changes.filter(v=>v===0).length,absoluteChangeRange:[Math.min(...changes),Math.max(...changes)],
          relativeReductionPercentRange:pct?[Math.min(...pct),Math.max(...pct)]:null};
      }
      groups[arm][String(steps)]=stats;
    }
  }
  const calculated={comparedWithOwn50Steps:groups,checkpointSelected:false,samplerSelected:false,rootCauseProven:false,formalVisualQualification:false};
  assert.deepEqual(calculated,replay.summary,"independent sampling summary mismatch");
  return {...calculated,images:replay.images,rowsChecked:36,pngBaselinesExact:12,limitations:replay.limitations};
}
...
export const validate = context => phase(context, async request => {
  ...
  assert.deepEqual(result.replay.sourcePackage,request.sourcePackage);
  verifyBindings(context.projectRoot,result.replay.images);
  const summary = summarizePairedSampling(boundJson(context.projectRoot, request.sourceResult), result.replay, context.outputRoot);
...
export function decidePairedSampling(summary,evidence) {
  assert.equal(summary.rowsChecked,36);assert.equal(summary.pngBaselinesExact,12);
  assert.equal(summary.samplerSelected,false);assert.equal(summary.checkpointSelected,false);assert.equal(summary.formalVisualQualification,false);
  const stats=[];
  assert.deepEqual(Object.keys(summary.comparedWithOwn50Steps).sort(),[...ARMS].sort());
  for(const arm of ARMS) {
    assert.deepEqual(Object.keys(summary.comparedWithOwn50Steps[arm]).sort(),["10","25"]);
    for(const steps of ["25","10"]){
      const m=summary.comparedWithOwn50Steps[arm][steps];
      assert.deepEqual(Object.keys(m).sort(),[...METRICS,...HEADS].sort());
      for(const value of Object.values(m)){
        for(const k of ["improvedCount","worseCount","equalCount"])assert(Number.isInteger(value[k])&&value[k]>=0&&value[k]<=6);
        assert.equal(value.improvedCount+value.worseCount+value.equalCount,6);stats.push(value);
      }
    }
  }
  const options=["short_grids_reduce_recorded_errors","mixed_short_grid_effect","short_grids_not_improved"];
  const option=stats.every(s=>s.improvedCount===6)?options[0]:stats.some(s=>s.improvedCount>0)?options[1]:options[2];
  return {...adjudicateBoundedDecision({decisionSetId:"paired-sampling-shadow",ruleVersion:"all-seed-all-metric-directions-v1",
    optionIds:options,matchedOptionIds:[option],evidenceReferences:[evidence],evidenceComplete:true}),
    disposition:"sampling_evidence_only_no_automatic_selection",applied:false,nextMachineAction:null,uniqueRootCauseProven:false,
    stopReason:"Shorter-grid comparisons do not select a sampler, qualify a model, or authorize more training."};
}
...
export const adjudicate = context => phase(context, async () => {
  ...
  return { status: "passed", artifact: persist(context.projectRoot, phaseFile(context, "adjudicate"), decidePairedSampling(reviewValue.summary, evidence)) };
});
...
export const finalize = context => phase(context, async request => {
  ...
  verifyBindings(context.projectRoot, reviewValue.summary.images);
  const artifact = persist(context.projectRoot, phaseFile(context, "finalize"), {
    schemaVersion: "ai-painter-paired-sampling-shadow-result-v1", runId: request.identity,
    status: "paired_sampling_shadow_completed_not_visual_qualified", executionState: "completed",
    ...
    migrationStatus: "shadow_validated_not_production_cutover", migratedFunctions: ["current_result_binding", "frozen_sampling_grid_comparison", "exact_source_png_replay", "independent_all_seed_metric_comparison", "bounded_sampling_advice", "transactional_finalization"], recordedAtUtc: new Date().toISOString() });
  verifyBindings(context.projectRoot, request.inputReceipts);
  remainingMilliseconds(context, request);
  await register(context, request, artifact, false);
  return { status: "passed", artifact };
