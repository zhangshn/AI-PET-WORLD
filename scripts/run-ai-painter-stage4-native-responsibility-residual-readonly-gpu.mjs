...
const TASK = "run_native_condition_encoder_responsibility_residual_readonly_gpu_qualification"
const NEXT_TASK = "compile_native_responsibility_residual_controlled_smoke_contract"
const RUN_ID = process.argv[2] ?? `stage4-native-responsibility-residual-gpu-${compactUtc()}-01`
...
const IDENTITIES = ["terrain_path_ground", "object_footprints", "object_tree", "object_rock", "object_vegetation"]
...
const configAudit = existing(cpu.configurationAudit.path)
const candidateContract = existing(cpu.candidateContract.path)

const root = inside(`.runtime/ai-painter/stage4-native-responsibility-residual-readonly-gpu/${RUN_ID}`)
...
try {
  ...
  const runner = existing("ml/ai-painter/scripts/run_stage4_native_responsibility_residual_readonly_gpu_qualification.py")
  const checker = existing("ml/ai-painter/scripts/check_stage4_native_responsibility_residual_cpu.py")
  ...
  assert.equal(checked.status, "stage4_native_responsibility_residual_cpu_support_passed")
  assert.deepEqual(checked.splitCounts, { train: 48, validation: 8, challenge: 4, regression: 4 })
  assert.deepEqual(checked.responsibilityIdentityOrder, IDENTITIES)
  ...
  writeJsonAtomic(files.ticket, {
    ...
    executionIdentity: {
      ...
      latentChannels: 12,
      responsibilityIdentityOrder: IDENTITIES,
    },
    bindings: {
  ...
  assert.equal(result.status, "native_responsibility_residual_readonly_gpu_qualification_succeeded")
  ...
  assert.deepEqual(report.responsibilityIdentityOrder, IDENTITIES)
  assert.equal(report.gradientEvidence.responsibilityHeadGradientTensorCount, 10)
  assert.equal(report.gradientEvidence.responsibilityIsolation.every((item) => item.outsideResidualNonzeroCount === 0), true)
  ...
  const lifecycle = advanceCapabilityLifecycle({
    ...
    evidence: {
      ...
      targetState: "readonly_gpu_qualified",
      status: "passed",
      bindings: [files.cpuEntry, gpuReport, gradients, states, cuda, diagnosticTerminal].map(bind),
    },
    recordedAtUtc,
  })
  const lifecycleState = existing(`.runtime/ai-painter/capability-lifecycle/${CAPABILITY}/state.json`)
  writeJsonAtomic(files.terminal, {
    schemaVersion: "stage4-native-responsibility-residual-readonly-gpu-package-terminal-v1",
  ...
  appendAiPainterProgramEvent({
    id: `stage4-native-responsibility-residual-gpu-${RUN_ID}`,
    timestamp: recordedAtUtc,
    action: "stage4_native_responsibility_residual_readonly_gpu_qualified",
    runId: RUN_ID,
    kind: "local_autonomous_readonly_gpu_qualification",
    status: "success",
    title: "Native responsibility residual readonly GPU qualification passed",
    titleZh: "Stage4原生条件责任残差只读GPU资格通过",
    detailZh: "真实validation样本194、五个掩码责任出口、CUDA梯度、掩码外零值与模型状态不变全部通过；未训练。",
  ...
  if (!fs.existsSync(failure)) {
    writeJsonAtomic(failure, {
      schemaVersion: "stage4-native-responsibility-residual-readonly-gpu-failure-v1",
      executionState: "failed_closed",
      status: "native_responsibility_residual_readonly_gpu_qualification_failed",
      runId: RUN_ID,
...
function capsule(recordedAtUtc, evidenceFiles) {
  return {
    ...
    currentStage: { number: 4, total: 5, labelZh: "Stage4原生条件责任残差只读GPU资格", status: "readonly_gpu_qualified" },
    candidateTerminal: {
      runId: RUN_ID,
      status: "completed",
      programStatus: "native_responsibility_residual_readonly_gpu_qualification_succeeded",
      previewMachineStatus: "not_run_readonly_gpu_only",
      modelQualificationStatus: "readonly_gpu_qualified",
      checkpointWritten: false,
      modelWeightsModified: false,
      recordedAtUtc,
    },
    latestBlocker: { code: "controlled_smoke_contract_not_compiled", summaryZh: "只读GPU资格通过，受控Smoke合同尚未编译。" },
    nextAllowedAction: { code: NEXT_TASK, labelZh: "编译责任残差30 Epoch受控Smoke合同", ownerAuthorizationRequired: false, automaticExecutionAllowed: true, planEvidenceConfirmed: true },
    ...
    taskIdentity: { modelId: "stage4_native_condition_encoder_masked_responsibility_residual_v1", sampleId: "ai-cold-start-v7-v7-capacity-slot-194-wet-season-drainage-hollow-v6", sampleSplit: "validation", seed: 20263722, requiredBoundarySides: ["west"], responsibilityIdentityOrder: IDENTITIES },
    evidence: evidenceFiles.map((file) => ({ kind: path.basename(file), labelZh: path.basename(file), ...bind(file), expectedSha256: sha(file), sha256Verified: true, recordedAtUtc, recordedAtAsiaShanghai: formatShanghai(recordedAtUtc) })),
...
function runPython(python, args) { const value = execFileSync(python, args, { cwd: ROOT, encoding: "utf8", maxBuffer: 64 * 1024 * 1024 }); const start = value.indexOf("{"); assert.ok(start >= 0, "Python JSON output missing"); return JSON.parse(value.slice(start)) }
