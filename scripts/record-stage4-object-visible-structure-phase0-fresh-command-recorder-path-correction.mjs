...
import { GPU_REQUEST_ID, buildGpuAuthorizationFixture, validateGpuAuthorizationDocument } from "./lib/ai-painter-stage4-object-visible-structure-phase0-execution.mjs"
import { appendAiPainterProgramEvent, formatShanghai, writeJsonAtomic } from "./lib/ai-painter-program-event-store.mjs"
import { indexArtifact } from "./lib/ai-pet-world-storage-catalog.mjs"
import { logicalProjectPath } from "./lib/ai-pet-world-storage.mjs"

const ROOT = process.cwd()
const REQUEST_ID = "owner-authorized-stage4-object-visible-structure-phase0-fresh-command-recorder-path-correction-20260815-063000000"
const AUTHORIZATION_SHA256 = "02ffea730d51ae951b5d0117f933e45fb74c18a35f096f11194bc375b01aa48a"
...
const OUTPUT = ".runtime/ai-painter/stage4-object-visible-structure-phase0-fresh-command-recorder-path-corrections/20260815-063000000"
...
assert.equal(authorization.schemaVersion, "ai-painter-owner-stage4-object-visible-structure-phase0-fresh-command-recorder-path-correction-v1")
...
assert.equal(authorization.scope, "one_cpu_only_bounded_recorder_identity_path_correction_and_existing_implementation_finalization")
assert.equal(consumption.status, "stage4_object_visible_structure_phase0_fresh_command_recorder_path_correction_authorization_atomically_consumed")
...
for (const [name, binding] of Object.entries(authorization.bindings)) {
  if (name === "failedRecorder") assert.equal(binding.sha256, "94cfa6663d2bc6768fcdca9d292d509cef8d7d1c8e66590d9353c0717772cfb8")
...
assert.equal(fs.existsSync(output), false, "recorder_path_correction_output_exists")
...
const correctedRecorder = projectFile(authorization.bindings.failedRecorder.path)
const recorder = projectFile("scripts/record-stage4-object-visible-structure-phase0-fresh-command-recorder-path-correction.mjs")
const trainer = projectFile(authorization.bindings.trainer.path)
for (const [name, file] of Object.entries({ phase0Runner: runner, phase0PythonEntry: pythonEntry, phase0CpuChecker: checker, phase0SharedLibrary: library, trainer })) assert.equal(sha(file), authorization.bindings[name].sha256, `${name}_must_remain_frozen`)
for (const file of [runner, checker, library, correctedRecorder, recorder]) { const result = spawnSync(process.execPath, ["--check", file], { cwd: ROOT, encoding: "utf8", env: { ...process.env, CUDA_VISIBLE_DEVICES: "" }, windowsHide: true }); assert.equal(result.status, 0, `node_syntax_failed:${relative(file)}:${result.stderr}`) }
...
const failedImplementationAuthorization = read(projectFile(authorization.bindings.failedImplementationAuthorization.path))
const failedGpuTerminalPath = projectFile(failedImplementationAuthorization.bindings.failedGpuTerminal.path)
const lineageIdentity = path.join(path.dirname(path.dirname(failedGpuTerminalPath)), "phase0-update-execution-identity.json")
assert.equal(fs.existsSync(lineageIdentity), true, "lineage_identity_missing")
const cpuRun = spawnSync(process.execPath, [checker,
  ...
  "--lineage-identity", relative(lineageIdentity),
...
writeJsonAtomic(files.cpu, { ...cpu, actualRunnerContract: entry, authorization: bind(authorizationPath), consumption: bind(consumptionPath), lineageIdentity: bind(lineageIdentity), recordedAtUtc: now, recordedAtAsiaShanghai: shanghai })
writeJsonAtomic(files.report, {
  schemaVersion: "stage4-object-visible-structure-phase0-fresh-command-recorder-path-correction-implementation-report-v1",
  status: "stage4_object_visible_structure_phase0_fresh_gpu_command_identity_implemented_cpu_verified",
  authorization: bind(authorizationPath), consumption: bind(consumptionPath), priorFailedImplementation: { authorization: authorization.bindings.failedImplementationAuthorization, consumption: authorization.bindings.failedImplementationConsumption, failureReport: authorization.bindings.failureReport, failureTerminal: authorization.bindings.failureTerminal },
  correction: { recorderPathConstruction: "path_join_from_bound_failure_terminal_parent", correctedRecorder: bind(correctedRecorder), runnerFrozen: true, sharedLibraryFrozen: true, pythonEntryFrozen: true, cpuCheckerFrozen: true },
  codeIdentity: { runner: bind(runner), pythonEntry: bind(pythonEntry), cpuChecker: bind(checker), sharedLibrary: bind(library), correctedRecorder: bind(correctedRecorder), finalizer: bind(recorder), trainer: bind(trainer) },
...
writeJsonAtomic(files.terminal, {
  schemaVersion: "stage4-object-visible-structure-phase0-fresh-command-recorder-path-correction-terminal-v1",
  status: "stage4_object_visible_structure_phase0_fresh_command_recorder_path_correction_succeeded_closed",
  implementationReport: bind(files.report), implementationAttestation: bind(files.attestation), cpuContractReport: bind(files.cpu), fixedTotalProgress: { completedStages: 3, totalStages: 5, percent: 60 }, nextLegalAction: "owner_create_new_bound_single_gpu_phase0_execution_authorization_or_exit",
...
const failedGpuAuthorization = read(projectFile(failedImplementationAuthorization.bindings.failedGpuAuthorization.path))
...
writeJsonAtomic(files.owner, { schemaVersion: "ai-painter-owner-action-request-preview-v1", status: "not_authorized_not_consumed", requestedAction: "owner_create_new_bound_single_gpu_phase0_execution_authorization_or_exit", requestedAuthorizationPath: `.runtime/ai-painter/owner-action-requests/${GPU_REQUEST_ID}/authorization.json`, proposedAuthorization, boundImplementationReport: bind(files.report), boundImplementationAttestation: bind(files.attestation), boundTerminal: bind(files.terminal), gpuExecutedNow: false, automaticApproval: false, recordedAtUtc: now, recordedAtAsiaShanghai: shanghai })
writeJsonAtomic(files.capsule, { schemaVersion: "ai-painter-local-task-capsule-v1", module: "AI Painter R5", fixedTotalProgress: { completedStages: 3, totalStages: 5, percent: 60 }, currentStage: "Fresh Phase0 GPU command identity and corrected recorder CPU-verified; GPU execution inactive", candidateTerminal: bind(files.terminal), latestBlocker: "new_immutable_owner_single_gpu_phase0_execution_authorization_not_created_or_consumed", nextLegalAction: "owner_create_new_bound_single_gpu_phase0_execution_authorization_or_exit", forbiddenActions: authorization.forbiddenActions, evidence: { implementationReport: bind(files.report), implementationAttestation: bind(files.attestation), cpuContractReport: bind(files.cpu), ownerActionRequest: bind(files.owner) }, gpuUsedNow: false, cudaInitializedNow: false, checkpointReadNow: false, modelLoadedNow: false, trainingStartedNow: false, recordedAtUtc: now, recordedAtAsiaShanghai: shanghai })
for (const file of [authorizationPath, consumptionPath, runner, pythonEntry, checker, library, correctedRecorder, recorder, trainer, ...Object.values(files)]) { const stat = fs.statSync(file); indexArtifact({ logicalPath: logicalProjectPath(file), physicalUri: fs.realpathSync(file), storageLayer: "hot", runId: REQUEST_ID, byteSize: stat.size, modifiedAtUtc: stat.mtime.toISOString(), sha256: sha(file) }) }
appendAiPainterProgramEvent({ id: `stage4-object-visible-structure-phase0-recorder-path-correction-${REQUEST_ID}`, timestamp: now, action: "stage4_object_visible_structure_phase0_fresh_command_recorder_path_correction", runId: REQUEST_ID, kind: "cpu_only_implementation_finalization", status: "success", title: "Fresh Phase0 command recorder path corrected", titleZh: "Phase0 新命令记录器路径修正完成", detailZh: `CPU 正向 ${cpu.positivePassed}/${cpu.positiveTotal}、反向 ${cpu.negativePassed}/${cpu.negativeTotal}；未启动 GPU、CUDA、模型或训练。`, evidencePath: relative(files.terminal), evidenceSha256: sha(files.terminal), fixedTotalProgress: { completedStages: 3, totalStages: 5, percent: 60 } })
