...
const CHECKER = "ml/ai-painter/scripts/check_ai_assisted_conditional_v5_repair.py"
const SOURCES = [
  ...
  CHECKER,
  "scripts/diagnose-ai-assisted-conditional-v4-failure.mjs",
]
const createdAtUtc = new Date().toISOString()
const runId = `ai-assisted-conditional-v5-cpu-regression-${createdAtUtc.replace(/[:.]/g, "-")}`
...
const passed = child.status === 0 && result?.status === "passed" && result?.gpuUsed === false && result?.imageGenerated === false
const record = {
  schemaVersion: "ai-assisted-conditional-v5-repair-cpu-regression-v1",
  status: passed ? "passed" : "failed",
  runId,
  createdAtUtc,
  createdAtAsiaShanghai: formatShanghai(createdAtUtc),
  ownerCommandRef: "owner-authorized-v4-diagnosis-and-repair-record-20260721",
  repairVersion: "V5",
  sourceV4DiagnosisPath: ".runtime/ai-painter/ai-assisted-conditional-repair-diagnostics/latest.json",
  result,
...
const written = writeImmutableProgramRun({
  root: ".runtime/ai-painter/ai-assisted-conditional-v5-repair-checks",
  runId,
  fileName: "report.json",
  record,
  latest: { repairVersion: "V5", gpuTrainingStarted: false, imageInferenceStarted: false },
})
appendAiPainterProgramEvent({
  action: "check_ai_assisted_conditional_v5_repair",
  runId,
  kind: passed ? "repair_cpu_regression_passed" : "repair_cpu_regression_failed",
  status: passed ? "success" : "failed",
  title: passed ? "V5 repair CPU regression passed" : "V5 repair CPU regression failed",
  titleZh: passed ? "V5 修复 CPU 回归已通过" : "V5 修复 CPU 回归失败",
  ...
  script: "scripts/check-ai-assisted-conditional-v5-repair.mjs",
  currentStep: "v5_repair_cpu_regression",
