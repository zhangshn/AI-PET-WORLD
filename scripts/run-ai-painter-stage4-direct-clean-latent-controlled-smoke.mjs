...
const TASK_ID = "execute_direct_condition_clean_latent_controlled_smoke_closed_loop";
const SAMPLE_ID = "ai-cold-start-v7-v7-capacity-slot-194-wet-season-drainage-hollow-v6";
const PREVIEW_EPOCHS = [1, 5, 10, 20, 30];
const CONTRACT = resolve(".runtime/ai-painter/stage4-direct-clean-latent-smoke-contract-compilations/stage4-direct-clean-latent-smoke-contract-20260827-01/controlled-smoke-contract.json");
const CONTRACT_SHA256 = "d889256caed6e1f2db19d0851270611a96286937775a9c5ff79c56c4c019f780";
const INACTIVE = resolve(".runtime/ai-painter/stage4-direct-clean-latent-readonly-gpu-qualifications/stage4-direct-clean-latent-readonly-gpu-20260827-03/inactive-config.json");
const INACTIVE_SHA256 = "3cbda9a420004e1f5851a047be2fa1db74a9b5df1d11689f7f76e96a817e5a99";
const SUPPORT_TERMINAL = resolve(".runtime/ai-painter/stage4-direct-clean-latent-smoke-trainer-support/stage4-direct-clean-latent-smoke-trainer-support-20260827-01/phase-terminal.json");
const SUPPORT_TERMINAL_SHA256 = "28770587f7c87df0b2719bc085504c366caa4f383df35d5a4daacda84d2c1ffc";
const TRAINER_PREFLIGHT = resolve(".runtime/ai-painter/stage4-direct-clean-latent-smoke-trainer-preflights/stage4-direct-clean-latent-smoke-trainer-preflight-20260827-01/preflight-report.json");
const TRAINER_PREFLIGHT_SHA256 = "1842bb165dd49c631b7d83bc6f23b107722fcdc495a512f3c8ef0c963a10512e";
...
const CPU_CHECKER = resolve("ml/ai-painter/scripts/check_stage4_direct_clean_latent_smoke_cpu.py");
const COMPILER = resolve("ml/ai-painter/scripts/compile_stage4_direct_clean_latent_smoke_active_config.py");
const EXECUTION_ROOT = resolve(`.runtime/ai-painter/stage4-direct-clean-latent-controlled-smokes/${RUN_ID}`);

const current = await readCurrentExecutionRegistry(ROOT);
assert.equal(current.ok, true, current.errorCode);
assert.equal(current.registry.capabilityVersion, CAPABILITY);
assert.equal(current.registry.packageId, RUN_ID);
...
verifyFile(CONTRACT, CONTRACT_SHA256, "controlled Smoke contract");
verifyFile(INACTIVE, INACTIVE_SHA256, "qualified inactive config");
verifyFile(SUPPORT_TERMINAL, SUPPORT_TERMINAL_SHA256, "trainer support terminal");
verifyFile(TRAINER_PREFLIGHT, TRAINER_PREFLIGHT_SHA256, "Node-to-Trainer preflight");
...
assert.equal(contract.executionIdentity.runId, RUN_ID);
assert.deepEqual(contract.executionIdentity.previewEpochs, PREVIEW_EPOCHS);
for (const evidence of contract.sourceEvidence) {
  verifyFile(resolve(evidence.path), evidence.sha256, `contract source evidence ${evidence.role}`);
}
...
const priorPreflight = read(TRAINER_PREFLIGHT);
assert.equal(priorPreflight.status, "node_to_trainer_readonly_preflight_passed");
assert.deepEqual(priorPreflight.safety, {
...
writeExclusive(paths.trainerPreflightBinding, {
  schemaVersion: "stage4-direct-clean-latent-execution-preflight-binding-v1",
  status: "verified",
  trainerSupportTerminal: bind(SUPPORT_TERMINAL),
  nodeTrainerPreflight: bind(TRAINER_PREFLIGHT),
  trainingOutputCreated: false,
  gpuStarted: false,
  optimizerCreated: false,
  backwardExecuted: false,
...
const ticket = {
  ...
  runId: RUN_ID,
  scope: "one_direct_clean_latent_30_epoch_controlled_smoke_closed_loop",
...
assert.equal(read(paths.activeConfig).training.directCleanLatentControlledSmoke.ticketState, "consumed");

writeJsonAtomic(paths.executionState, {
  schemaVersion: "stage4-direct-clean-latent-controlled-smoke-execution-state-v1",
  status: "running",
  phase: "training",
  capabilityVersion: CAPABILITY,
  runId: RUN_ID,
...
const heartbeat = () => {
  ...
  writeJsonAtomic(paths.monitorTelemetry, {
    schemaVersion: "stage4-direct-clean-latent-smoke-monitor-resource-telemetry-v1",
    status: "recording",
    rows: telemetryRows,
  ...
  process.stdout.write(`${JSON.stringify({
    kind: "direct_clean_latent_smoke_heartbeat",
    runId: RUN_ID,
    phase: progress?.phase ?? "initializing",
...
writeExclusive(paths.manifest, {
  schemaVersion: "stage4-direct-clean-latent-controlled-smoke-root-manifest-v1",
...
const terminalStatus = qualification.qualified
  ? "direct_clean_latent_controlled_smoke_qualified"
  : "direct_clean_latent_controlled_smoke_real_visual_failure";
const nextLegalAction = qualification.qualified
  ? "compile_direct_condition_clean_latent_stage0"
  : "record_direct_condition_clean_latent_smoke_failure_and_close";
writeExclusive(paths.finalization, {
  schemaVersion: "stage4-direct-clean-latent-controlled-smoke-finalization-v1",
...
writeExclusive(paths.terminal, {
  schemaVersion: "stage4-direct-clean-latent-controlled-smoke-terminal-v1",
...
appendAiPainterProgramEvent({
  id: `stage4-direct-clean-latent-controlled-smoke-${RUN_ID}`,
  timestamp: new Date().toISOString(),
  action: "stage4_direct_clean_latent_controlled_smoke_closed_loop",
  runId: RUN_ID,
  ...
  titleZh: qualification.qualified
    ? "Stage4直达干净潜变量受控Smoke及自动审核通过"
    : "Stage4直达干净潜变量受控Smoke真实视觉失败并关闭",
...
async function reviewPreviews() {
  ...
  for (const artifact of manifest.fixedPreviews) {
    ...
    const normalized = await normalizePreviewWithWindowsSafeIo({
      ...
      workRoot: resolve(".runtime/ai-painter/direct-clean-latent-review-work"),
    ...
    const [aesthetic, alignment] = await Promise.all([
      ...
      auditAiAssistedConditionAlignment({
        record: {
          recordId: `direct-clean-latent-smoke-${epoch}`,
          conditionBinding: {
    ...
    writeJsonAtomic(paths.reviewProgress, {
      schemaVersion: "stage4-direct-clean-latent-machine-review-progress-v1",
      status: "running",
      runId: RUN_ID,
  ...
  const report = {
    schemaVersion: "stage4-direct-clean-latent-machine-review-v1",
...
function qualifyLateStability(review) {
  ...
  return {
    schemaVersion: "stage4-direct-clean-latent-late-stability-qualification-v1",
...
async function closeFailure(code, detail, trainingStarted) {
  writeExclusive(paths.failure, {
    schemaVersion: "stage4-direct-clean-latent-controlled-smoke-failure-v1",
  ...
  writeExclusive(paths.terminal, {
    schemaVersion: "stage4-direct-clean-latent-controlled-smoke-terminal-v1",
    executionState: "completed",
    status: "direct_clean_latent_controlled_smoke_infrastructure_failed_closed",
    capabilityVersion: CAPABILITY,
    runId: RUN_ID,
    ...
    ownerAuthorizationRequired: false,
    fixedTotalProgress: { completedStages: 3, totalStages: 5, percent: 60 },
    nextLegalAction: "repair_direct_clean_latent_smoke_infrastructure_from_saved_evidence",
    recordedAtUtc: new Date().toISOString(),
  });
  writeJsonAtomic(paths.executionState, {
    schemaVersion: "stage4-direct-clean-latent-controlled-smoke-execution-state-v1",
  ...
  appendAiPainterProgramEvent({
    id: `stage4-direct-clean-latent-smoke-failure-${RUN_ID}`,
    timestamp: new Date().toISOString(),
    action: "stage4_direct_clean_latent_controlled_smoke_closed_loop",
    runId: RUN_ID,
    kind: "controlled_smoke",
    ...
    titleZh: "Stage4直达干净潜变量受控Smoke基础设施失败关闭",
    detailZh: `${code}；未自动重试。`,
    evidencePath: projectPath(paths.terminal),
    evidenceSha256: sha256(paths.terminal),
    fixedTotalProgress: { completedStages: 3, totalStages: 5, percent: 60 },
...
function resourceSnapshot() {
  ...
  return {
    schemaVersion: "stage4-direct-clean-latent-resource-preflight-v1",
