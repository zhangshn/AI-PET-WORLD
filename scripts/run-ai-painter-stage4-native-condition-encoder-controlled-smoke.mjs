...
const TASK_ID = "run_native_condition_encoder_fixed_40_epoch_qualification"
const RUN_ID = `stage4-native-condition-encoder-fixed-40-epoch-${compactUtc()}-01`
...
const INACTIVE = resolve(".runtime/ai-painter/stage4-native-condition-encoder-cpu-support/stage4-native-condition-encoder-cpu-20260827125945-01/inactive-config.json")
const GPU_TERMINAL = resolve(".runtime/ai-painter/stage4-native-condition-encoder-readonly-gpu/stage4-native-condition-encoder-gpu-20260827131853-01/phase-terminal.json")
...
const CPU_CHECKER = resolve("ml/ai-painter/scripts/check_stage4_native_condition_encoder_smoke_cpu.py")
const COMPILER = resolve("ml/ai-painter/scripts/compile_stage4_native_condition_encoder_smoke_active_config.py")
const CONTRACT_ROOT = resolve(`.runtime/ai-painter/stage4-native-condition-encoder-fixed-40-epoch-contracts/${RUN_ID}`)
const CONTRACT = path.join(CONTRACT_ROOT, "controlled-smoke-contract.json")
const EXECUTION_ROOT = resolve(`.runtime/ai-painter/stage4-native-condition-encoder-fixed-40-epoch-qualifications/${RUN_ID}`)
...
const ANALYSIS_TERMINAL = resolve(current.registry.terminalEvidence.path)
verifyFile(ANALYSIS_TERMINAL, current.registry.terminalEvidence.sha256, "current causal analysis terminal")
const analysisTerminal = read(ANALYSIS_TERMINAL)
assert.equal(analysisTerminal.status, "native_condition_encoder_smoke_failure_analysis_succeeded")
const QUALIFICATION_CONTRACT = resolve(analysisTerminal.boundedQualificationContract.path)
verifyFile(QUALIFICATION_CONTRACT, analysisTerminal.boundedQualificationContract.sha256, "bounded 40 Epoch qualification contract")
...
verifyFile(GPU_TERMINAL, "2ab6bff977f108fd4af452a9892c1c126d328dc4b6d015db8cc17dfd26efe73c", "qualified readonly GPU terminal")

const sourceEvidence = [
  ["readonlyGpuTerminal", GPU_TERMINAL],
  ["causalAnalysisTerminal", ANALYSIS_TERMINAL],
  ["bounded40EpochQualificationContract", QUALIFICATION_CONTRACT],
  ...
  ["structureContract", resolve("ml/ai-painter/scripts/ai_painter_native_condition_encoder_contract.py")],
...
assert.equal(smokeCpu.status, "stage4_native_condition_encoder_smoke_cpu_gate_passed")
writeExclusive(path.join(CONTRACT_ROOT, "cpu-report.json"), { schemaVersion: "stage4-native-condition-encoder-smoke-contract-cpu-report-v1", status: "passed", ...smokeCpu, contract: bind(CONTRACT), sourceEvidence, ownerAuthorizationRequired: false, recordedAtUtc: new Date().toISOString() })
writeExclusive(path.join(CONTRACT_ROOT, "evidence-isolation-audit.json"), { schemaVersion: "stage4-native-condition-encoder-smoke-evidence-isolation-audit-v1", status: "passed", currentCapabilityOnly: true, historicalSmokeRead: false, historicalCheckpointRead: false, crossRunOutputReuse: false, sourceEvidence, recordedAtUtc: new Date().toISOString() })
...
runSync(PYTHON, [COMPILER, "--inactive-config", INACTIVE, "--smoke-contract", CONTRACT, "--ticket-state", "preflight_unconsumed", "--output", paths.preflightConfig], 300_000)
...
runSync(PYTHON, [COMPILER, "--inactive-config", INACTIVE, "--smoke-contract", CONTRACT, "--ticket-state", "consumed", "--output", paths.activeConfig], 300_000)
assert.equal(read(paths.activeConfig).training.nativeConditionEncoderControlledSmoke.ticketState, "consumed")

writeJsonAtomic(paths.executionState, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-execution-state-v1", status: "running", phase: "training", capabilityVersion: CAPABILITY, runId: RUN_ID, trainingOutput: projectPath(paths.trainingOutput), progressPath: projectPath(paths.progress), automaticReviewAfterTraining: true, ownerAuthorizationRequired: false, startedAtUtc: new Date().toISOString() })
...
const heartbeat = () => {
  ...
  writeJsonAtomic(paths.monitorTelemetry, { schemaVersion: "stage4-native-condition-encoder-smoke-monitor-resource-telemetry-v1", status: "recording", rows: telemetryRows, peakGpuMemoryMiB: Math.max(...telemetryRows.map((row) => row.memoryUsedMiB || 0)), recordedAtUtc: new Date().toISOString() })
  process.stdout.write(`${JSON.stringify({ kind: "native_condition_encoder_smoke_heartbeat", runId: RUN_ID, phase: progress?.phase ?? "initializing", epoch: progress?.currentEpoch ?? 0, epochTarget: progress?.epochTarget ?? EPOCH_COUNT, optimizerStep: progress?.optimizerStep ?? 0, optimizerStepTarget: progress?.optimizerStepTarget ?? EPOCH_COUNT, percent: progress?.percent ?? 0, etaSeconds: progress?.etaSeconds ?? null, elapsedSeconds: Math.round((Date.now() - startedAt) / 1000), gpu, recordedAtUtc: new Date().toISOString() })}\n`)
}
heartbeat()
...
writeJsonAtomic(paths.executionState, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-execution-state-v1", status: "running", phase: "automatic_machine_review", capabilityVersion: CAPABILITY, runId: RUN_ID, trainingOutput: projectPath(paths.trainingOutput), ownerAuthorizationRequired: false, updatedAtUtc: new Date().toISOString() })
...
writeExclusive(paths.manifest, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-root-manifest-v1", status: qualification.qualified ? "qualified" : "real_visual_failure", runId: RUN_ID, capabilityVersion: CAPABILITY, controlledSmokeContract: bind(CONTRACT), activeConfig: bind(paths.activeConfig), internalTicket: bind(paths.ticket), internalTicketConsumption: bind(paths.consumption), trainingManifest: bind(paths.trainingManifest), progress: bind(paths.progress), checkpoint: trainingManifest.checkpoint, machineReview: bind(paths.machineReview), lateStabilityQualification: bind(paths.qualification), resourceTelemetry: bind(paths.trainingTelemetry), monitorResourceTelemetry: bind(paths.monitorTelemetry), modelWeightsModified: true, checkpointPromotable: false, automaticRetryStarted: false, stage0Started: false, ownerAuthorizationRequired: false, recordedAtUtc: new Date().toISOString() })
const terminalStatus = qualification.qualified ? "native_condition_encoder_fixed_40_epoch_qualification_succeeded" : "native_condition_encoder_fixed_40_epoch_qualification_real_visual_failure"
const nextLegalAction = qualification.qualified ? "compile_native_condition_encoder_stage0" : "retire_native_condition_encoder_candidate_after_fixed_40_epoch_failure"
writeExclusive(paths.finalization, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-finalization-v1", status: terminalStatus, runId: RUN_ID, manifest: bind(paths.manifest), trainingManifest: bind(paths.trainingManifest), machineReview: bind(paths.machineReview), lateStabilityQualification: bind(paths.qualification), checkpoint: trainingManifest.checkpoint, checkpointPromotable: false, automaticRetryStarted: false, stage0Started: false, ownerAuthorizationRequired: false, recordedAtUtc: new Date().toISOString() })
writeExclusive(paths.terminal, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-terminal-v1", executionState: "completed", status: terminalStatus, capabilityVersion: CAPABILITY, runId: RUN_ID, finalization: bind(paths.finalization), manifest: bind(paths.manifest), machineReview: bind(paths.machineReview), lateStabilityQualification: bind(paths.qualification), resourceTelemetry: bind(paths.trainingTelemetry), checkpointWritten: true, checkpointPromotable: false, modelWeightsModified: true, trainingStarted: true, automaticRetryStarted: false, stage0Started: false, ownerAuthorizationRequired: false, fixedTotalProgress: progressFixed(), nextLegalAction, recordedAtUtc: new Date().toISOString() })
writeJsonAtomic(paths.executionState, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-execution-state-v1", status: "completed", phase: qualification.qualified ? "qualified" : "failed_closed", capabilityVersion: CAPABILITY, runId: RUN_ID, terminal: bind(paths.terminal), ownerAuthorizationRequired: false, completedAtUtc: new Date().toISOString() })
writeExclusive(paths.capsule, buildCapsule(terminalStatus, nextLegalAction, review, qualification.qualified))
if (qualification.qualified) advanceCapabilityLifecycle({ root: ROOT, capabilityVersion: CAPABILITY, targetState: "controlled_smoke_completed", evidence: { schemaVersion: "ai-painter-capability-stage-evidence-v1", capabilityVersion: CAPABILITY, targetState: "controlled_smoke_completed", status: "passed", bindings: [paths.terminal, paths.finalization, paths.manifest, paths.machineReview, paths.qualification].map(bind), ownerAuthorizationRequired: false } })
const advanced = await advanceCurrentExecutionRegistry({ projectRoot: ROOT, capabilityVersion: CAPABILITY, packageId: RUN_ID, taskId: nextLegalAction, taskKind: qualification.qualified ? "formal_stage_training" : "cpu_readonly_recording", runId: RUN_ID, lifecycleStage: qualification.qualified ? "controlled_smoke_completed" : "readonly_gpu_qualified", executionState: "package_materialized", activity: "planned_not_started", taskCapsulePath: projectPath(paths.capsule), terminalEvidencePath: projectPath(paths.terminal), latestTrainingTerminal: { runId: RUN_ID, path: projectPath(paths.terminal), sha256: sha256(paths.terminal), status: terminalStatus, evidence: { executionState: bind(paths.executionState), machineReview: bind(paths.machineReview), reviewProgress: bind(paths.reviewProgress), trainingProgress: bind(paths.progress) } } })
appendAiPainterProgramEvent({ id: `stage4-native-condition-encoder-controlled-smoke-${RUN_ID}`, timestamp: new Date().toISOString(), action: "stage4_native_condition_encoder_fixed_40_epoch_qualification_closed_loop", runId: RUN_ID, kind: "controlled_smoke", status: qualification.qualified ? "success" : "failed_closed", title: "Stage4 native-condition encoder fixed 40 Epoch qualification completed", titleZh: qualification.qualified ? "Stage4原生条件编码固定40 Epoch资格通过" : "Stage4原生条件编码固定40 Epoch资格真实视觉失败并关闭", detailZh: `${EPOCH_COUNT} Epoch训练完成；机器审核${review.previewPassCount}/${review.previewCount}，资格=${qualification.qualified}。`, evidencePath: projectPath(paths.terminal), evidenceSha256: sha256(paths.terminal), fixedTotalProgress: progressFixed() })
...
async function reviewPreviews() {
  ...
  for (const artifact of manifest.fixedPreviews) {
    ...
    const normalized = await normalizePreviewWithWindowsSafeIo({ sourcePath, finalAssetPath: normalizedPath, workRoot: resolve(".runtime/ai-painter/native-condition-encoder-review-work"), workId: shaText(RUN_ID).slice(0, 16), epoch })
    const [aesthetic, alignment] = await Promise.all([
      auditAiAssistedProfessionalAesthetic(normalized.shortOutputPath),
      auditAiAssistedConditionAlignment({ record: { recordId: `native-condition-encoder-smoke-${epoch}`, conditionBinding: { conditionPackPath: sample.conditionPackPath, worldId: conditionPack.worldId, tick: conditionPack.tick }, classification: sample.classification }, imagePath: normalized.shortOutputPath, referenceImagePath: resolve(sample.imagePath) }),
    ])
...
function qualifyLateStability(review) {
  const decision = adjudicateLateReviewRows(review.reviews, {
    ...
    lateEpochs: [10, 20, 30, 40],
  })
  return { schemaVersion: "stage4-native-condition-encoder-late-stability-qualification-v1", status: decision.qualified ? "terminal_pass_with_late_convergence_evidence" : "late_stability_not_qualified", runId: RUN_ID, route: decision.qualificationRoute === "none" ? null : decision.qualificationRoute, lateEpochs: decision.issueSequence.map((row) => ({ epoch: row.epoch, passed: row.passed, failureCount: row.issueCodes.length, failureItems: [...row.issueCodes] })), exactRouteCount: Number(decision.sustainedZeroFromFirstLateEpoch) + Number(decision.strictDecreaseThenStableZero), consecutiveTerminalPasses: decision.issueSequence.at(-2).passed && decision.issueSequence.at(-1).passed, noTerminalRegression: decision.noRegression, finalPreviewByteReproductionValid: review.reviews.find((row) => row.epoch === EPOCH_COUNT)?.byteExactReproduced === true, qualified: decision.qualified, thresholdsChanged: false, recordedAtUtc: new Date().toISOString() }
}

async function closeInfrastructureFailure(code, detail, trainingStarted) {
  writeExclusive(paths.failure, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-failure-v1", status: "failed_closed", code, detail: String(detail), runId: RUN_ID, automaticRetryStarted: false, ownerAuthorizationRequired: false, recordedAtUtc: new Date().toISOString() })
  writeExclusive(paths.terminal, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-terminal-v1", executionState: "completed", status: "native_condition_encoder_controlled_smoke_infrastructure_failed_closed", capabilityVersion: CAPABILITY, runId: RUN_ID, failureReport: bind(paths.failure), trainingStarted, modelWeightsModified: trainingStarted, checkpointWritten: false, automaticRetryStarted: false, ownerAuthorizationRequired: false, fixedTotalProgress: progressFixed(), nextLegalAction: "repair_native_condition_encoder_smoke_infrastructure_from_saved_evidence", recordedAtUtc: new Date().toISOString() })
  writeJsonAtomic(paths.executionState, { schemaVersion: "stage4-native-condition-encoder-controlled-smoke-execution-state-v1", status: "completed", phase: "failed_closed", runId: RUN_ID, terminal: bind(paths.terminal), completedAtUtc: new Date().toISOString() })
