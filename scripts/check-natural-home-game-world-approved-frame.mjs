...
function assertNoBlockedTags(tags, label) {
  const tagSet = new Set(tags ?? [])
  const found = BLOCKED_TAGS.filter((tag) => tagSet.has(tag))
  assert(found.length === 0, `${label} contains blocked tags: ${found.join(", ")}`)
}

function assertOwnerFinalApprovalTags(tags, label) {
  const tagSet = new Set(tags ?? [])
  const missing = OWNER_FINAL_WORLD_APPROVAL_TAGS.filter((tag) => !tagSet.has(tag))
  assert(missing.length === 0, `${label} missing owner final approval tags: ${missing.join(", ")}`)
}
...
async function main() {
  ...
  assert(record.canShowToPlayer === true, "ApprovedFrame record should be displayable only after game-world gate")
  assert(frame.canShowToPlayer === true, "ApprovedFrame should be displayable only after game-world gate")
  assert(frame.approvalScope === "approved_for_game_world", "approval scope must be game-world")
  ...
  assert(frame.vj2Status === "vj_2_passed", "frame VJ-2 must pass")
  assert(review?.vj0Status === "vj_0_passed", "review VJ-0 must pass")
  assert(review?.vj1Status === "vj_1_passed", "review VJ-1 must pass")
  assert(review?.vj2Status === "vj_2_passed", "review VJ-2 must pass")
  assert(review?.approvalScope === "approved_for_game_world", "review approval scope must be game-world")
  assert(
    review?.checks?.some((check) => check.id === "game_world_approved_frame_protocol" && check.passed === true),
    "game-world ApprovedFrame evidence must be bound",
  )
  ...
  assert(frame.width === FORMAL_FRAME_WIDTH, "formal frame width mismatch")
  assert(frame.height === FORMAL_FRAME_HEIGHT, "formal frame height mismatch")
  assert(imageMetadata.width === FORMAL_FRAME_WIDTH, "real PNG width mismatch")
  assert(imageMetadata.height === FORMAL_FRAME_HEIGHT, "real PNG height mismatch")
  assert(imageMetadata.format === "png", "real image format mismatch")
  assert(approvedIndex.sourceImageSha256 === frame.sourceImageSha256, "index image sha mismatch")
  assert(approvedIndex.approvalScope === "approved_for_game_world", "index approval scope mismatch")
  assert(approvedIndex.vj2Status === "vj_2_passed", "index VJ-2 status mismatch")
  assert(approvedIndex.path === path.resolve(approvedIndex.path), "approved index path should be absolute")
  assert(frame.tags.includes("game_world_ready_for_player"), "frame missing game-world tag")
  assert(frame.tags.includes("formal_full_world_frame"), "frame missing formal-world-frame tag")
  assert(record.tags.includes("game_world_ready_for_player"), "record missing game-world tag")
  assert(record.tags.includes("formal_full_world_frame"), "record missing formal-world-frame tag")
  assertOwnerFinalApprovalTags(frame.tags, "frame")
  assertOwnerFinalApprovalTags(record.tags, "record")
  assertOwnerFinalApprovalTags(approvedIndex.tags, "index")
  assertNoBlockedTags(frame.tags, "frame")
  assertNoBlockedTags(record.tags, "record")
  assertNoBlockedTags(approvedIndex.tags, "index")

  console.log(
    `Natural Home Game-World ApprovedFrame check passed: ${frame.frameId}, ${record.sourceFactIds.length} source facts, ${imageBytes.length} bytes.`,
  )
}
