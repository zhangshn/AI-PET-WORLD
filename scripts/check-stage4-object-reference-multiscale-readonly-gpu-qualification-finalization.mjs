...
const TARGETS = [
  "scripts/finalize-stage4-object-reference-multiscale-readonly-gpu-qualification.mjs",
  "scripts/check-stage4-object-reference-multiscale-readonly-gpu-qualification-finalization.mjs",
]
...
assert.equal(
  ...
  "ai-painter-owner-stage4-object-reference-multiscale-gpu-qualification-continuation-v1",
)
...
function validateEvidence(candidate) {
  ...
  return (
    terminal.status === "stage4_four_object_reference_multiscale_gpu_qualification_passed_closed"
    && report.status === "passed_readonly_stage4_four_object_reference_multiscale_gpu_gradient_qualification"
    ...
    && report.identity?.trainingObjectiveContractId === "typed_object_multiscale_luminance_structure_correlation_supervision_v1"
...
const positive = {
  ...
  gpuTerminalPassed: source.gpuTerminal.status === "stage4_four_object_reference_multiscale_gpu_qualification_passed_closed",
  diagnosticReportPassed: source.diagnosticReport.status === "passed_readonly_stage4_four_object_reference_multiscale_gpu_gradient_qualification",
  ...
  matchingExpertGradientsFiniteNonzero: CHANNELS.every(
    (name) => source.diagnosticReport.gradientEvidence.fourObjectVisibleStructure[name].matchingSemanticMixtureExpertGradientNorm > 0,
  ),
  ...
  tenCompletedStepsPresent: source.diagnosticReport.completedSteps.length === 10,
}
...
const negativeMutations = {
  ...
  rejectCombinedZeroGradient: (x) => { x.diagnosticReport.gradientEvidence.fourObjectVisibleStructure.combined.denoiserGradientNorm = 0 },
...
const report = {
  schemaVersion: "stage4-object-reference-multiscale-gpu-qualification-finalization-cpu-contract-report-v1",
  status: failedPositiveKeys.length === 0 && failedNegativeKeys.length === 0
    ? "object_reference_multiscale_gpu_qualification_finalization_cpu_contract_passed"
    : "object_reference_multiscale_gpu_qualification_finalization_cpu_contract_failed_closed",
  positive,
  negative,
