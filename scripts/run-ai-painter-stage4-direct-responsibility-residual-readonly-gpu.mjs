...
const TASK = "run_direct_responsibility_residual_readonly_gpu_qualification"
const NEXT_TASK = "compile_direct_responsibility_residual_controlled_smoke_contract"
const RUN_ID = process.argv[2] ?? `stage4-direct-responsibility-residual-gpu-${compactUtc()}-01`
...
const cpu = JSON.parse(fs.readFileSync(cpuTerminal, "utf8"))
...
const root = inside(`.runtime/ai-painter/stage4-direct-responsibility-residual-readonly-gpu/${RUN_ID}`)
...
const files = {
  ...
  correction: path.join(root, "dataset-selection-correction.json"),
...
try {
  ...
  const runner = existing("ml/ai-painter/scripts/run_stage4_direct_responsibility_residual_readonly_gpu_qualification.py")
  ...
  writeJsonAtomic(files.correction, {
    schemaVersion: "stage4-direct-responsibility-residual-dataset-selection-correction-v1",
    status: "corrected_and_cpu_verified",
    cause: "new_architecture_identity_was_not_yet_registered_in_formal_v7_capacity_dataset_selection",
    beforeSelection: "current_condition_identity_v1_16_2_2_2",
    afterSelection: checked.datasetSelectionContract,
    splitCounts: checked.splitCounts,
    changedScope: "architecture_selection_predicate_and_cpu_checker_only",
    dataChanged: false,
    lossChanged: false,
  ...
  writeJsonAtomic(files.cpuEntry, { ...checked, status: "passed", trainer: bind(trainer), checker: bind(checker), correction: bind(files.correction), recordedAtUtc: new Date().toISOString() })
  writeJsonAtomic(files.ticket, {
    ...
    bindings: { inactiveConfig: bind(inactiveConfig), cpuTerminal: bind(cpuTerminal), cpuReport: bind(cpuReport), modelSupportContract: bind(support), cpuEntryReport: bind(files.cpuEntry), datasetSelectionCorrection: bind(files.correction), trainer: bind(trainer), modelFactory: bind(model), modeRegistry: bind(modeRegistry), sourceIndex: bind(sourceIndex), projectAutoencoderCheckpoint: bind(autoencoder) },
  ...
  assert.equal(result.status, "direct_responsibility_residual_readonly_gpu_qualification_succeeded")
  ...
  const lifecycle = advanceCapabilityLifecycle({ root: ROOT, capabilityVersion: CAPABILITY, targetState: "readonly_gpu_qualified", evidence: { schemaVersion: "ai-painter-capability-stage-evidence-v1", capabilityVersion: CAPABILITY, targetState: "readonly_gpu_qualified", status: "passed", bindings: [files.correction, files.cpuEntry, gpuReport, gradients, states, cuda, diagnosticTerminal].map(bind) }, recordedAtUtc: new Date().toISOString() })
  const lifecycleState = existing(`.runtime/ai-painter/capability-lifecycle/${CAPABILITY}/state.json`)
  const recordedAtUtc = new Date().toISOString()
  writeJsonAtomic(files.terminal, {
    schemaVersion: "stage4-direct-responsibility-residual-readonly-gpu-package-terminal-v1",
    ...
    datasetSelectionCorrection: bind(files.correction),
  ...
  writeJsonAtomic(files.capsule, capsule(recordedAtUtc, [files.terminal, files.ticket, files.consumption, files.correction, files.cpuEntry, files.preflight, gpuReport, gradients, states, cuda]))
  ...
  appendAiPainterProgramEvent({ id: `stage4-direct-responsibility-residual-gpu-${RUN_ID}`, timestamp: recordedAtUtc, action: "stage4_direct_responsibility_residual_readonly_gpu_qualified", runId: RUN_ID, kind: "local_autonomous_readonly_gpu_qualification", status: "success", title: "Direct responsibility residual readonly GPU qualification passed", titleZh: "Stage4责任残差只读GPU资格通过", detailZh: "真实validation样本194、正式48/8/4/4数据身份、CUDA前向、五责任梯度隔离及模型状态不变均通过；未训练。", evidencePath: relative(files.terminal), evidenceSha256: sha(files.terminal), fixedTotalProgress: progress() })
