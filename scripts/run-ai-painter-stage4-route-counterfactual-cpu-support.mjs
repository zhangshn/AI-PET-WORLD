...
const TASK = "implement_route_counterfactual_compositor_cpu_inactive_support"
const NEXT = "run_route_counterfactual_compositor_readonly_gpu_qualification"
const RUN_ID = `stage4-route-counterfactual-cpu-${compactUtc()}-01`
const OUTPUT = inside(`.runtime/ai-painter/stage4-route-counterfactual-cpu-support/${RUN_ID}`)
const PYTHON = inside("ml/ai-painter/.venv/Scripts/python.exe")
const COMPILER = inside("ml/ai-painter/scripts/compile_stage4_route_counterfactual_compositor_cpu_config.py")
const CHECKER = inside("ml/ai-painter/scripts/check_stage4_route_counterfactual_compositor_cpu.py")
const SOURCE_CONFIG = inside(
  ".runtime/ai-painter/stage4-direct-responsibility-residual-formal-stage0/" +
  "stage4-direct-responsibility-residual-stage0-20260827091911-01/active-config.json",
)
...
const IMPLEMENTATION_FILES = [
  ...
  inside("ml/ai-painter/scripts/ai_painter_route_counterfactual_compositor_contract.py"),
  COMPILER,
  CHECKER,
]
...
verifyFile(planTerminal, current.registry.terminalEvidence.sha256, "route plan terminal")
const plan = read(planTerminal)
const candidateContract = inside(plan.architectureContract.path)
verifyFile(candidateContract, plan.architectureContract.sha256, "route architecture contract")
...
assert.equal(cpu.status, "stage4_route_counterfactual_compositor_cpu_support_passed")
const recordedAtUtc = new Date().toISOString()
writeExclusive(report, {
  ...cpu,
  schemaVersion: "stage4-route-counterfactual-compositor-cpu-report-v1",
  candidateContract: bind(candidateContract),
  inactiveConfig: bind(inactive),
  ownerAuthorizationRequired: false,
  recordedAtUtc,
})
writeExclusive(audit, {
  schemaVersion: "stage4-route-counterfactual-compositor-configuration-audit-v1",
  status: "passed",
  architecture: cpu.architecture,
  parameterCount: cpu.parameterCount,
  additionalTrainableParameterCount: cpu.additionalTrainableParameterCount,
  sharedParameterCopies: cpu.sharedParameterCopies,
  counterfactualDerivedChannels: cpu.counterfactualDerivedChannels,
...
writeExclusive(terminal, {
  schemaVersion: "stage4-route-counterfactual-compositor-cpu-terminal-v1",
  executionState: "completed",
  status: "route_counterfactual_compositor_cpu_support_succeeded",
  capabilityVersion: CAPABILITY,
  runId: RUN_ID,
  candidateContract: bind(candidateContract),
  inactiveConfig: bind(inactive),
  cpuReport: bind(report),
  configurationAudit: bind(audit),
...
writeExclusive(capsule, {
  ...
  currentStage: { number: 4, total: 5, labelZh: "共享权重道路反事实CPU支持", status: "completed" },
  nextAllowedAction: { code: NEXT, ownerAuthorizationRequired: false, automaticExecutionAllowed: true },
...
advanceCapabilityLifecycle({
  ...
  evidence: {
    ...
    targetState: "cpu_contract_verified",
    status: "passed",
    bindings: [terminal, report, inactive, audit].map(bind),
    ownerAuthorizationRequired: false,
  },
})
...
appendAiPainterProgramEvent({
  id: `stage4-route-counterfactual-cpu-${RUN_ID}`,
  timestamp: recordedAtUtc,
  action: "stage4_route_counterfactual_compositor_cpu_support_completed",
  runId: RUN_ID,
  kind: "cpu_inactive_support",
  status: "success",
  title: "Route counterfactual compositor CPU support completed",
  titleZh: "共享权重道路反事实CPU支持完成",
  detailZh: `正向${cpu.positiveChecks}项、反向${cpu.negativeChecks}项通过；未启动GPU或训练。`,
...
function run(command, args) {
  const result = spawnSync(command, args, {
    cwd: ROOT,
    env: { ...process.env, PYTHONUTF8: "1", PYTHONPATH: `${inside("ml/ai-painter/src")};${inside("ml/ai-painter/scripts")}` },
    encoding: "utf8",
    windowsHide: true,
...
function verifyFile(file, expected, label) {
  ...
  assert.equal(sha(file), expected, `${label} SHA mismatch`)
}
