...
function writeApprovedFrame() {
  ...
  const candidate = {
    ...
    generationNotes: text(
      "由 V91 本地小模型生成，经 VJ-1 与最小 VJ-2 审核，再进入 ApprovedFrame 写入闸门。",
      "Generated by the V91 local small model, reviewed by VJ-1 and minimal VJ-2, then entered the ApprovedFrame write gate.",
    ),
    worldId: world.worldId,
    tick: world.tick,
    tags: [
  ...
  const reviewReport = {
    ...
    vj1QualitySummary: {
      status: "vj_1_passed",
      sampleWidth: candidateInfo.width,
      sampleHeight: candidateInfo.height,
      meanLuminance: 0,
  ...
  const approvedFrame = {
    ...
    sourceImageByteLength: imageBytes.length,
  ...
  assert(candidateInfo.imageSha256 === imageSha256, "image sha mismatch with binding")
  assert(candidateInfo.imageByteLength === imageBytes.length, "image byte length mismatch with binding")
...
const result = writeApprovedFrame()
console.log(
  [
