...
const runId = "20260813-075000000"
const output = path.join(root, ".runtime", "ai-painter", "stage4-terminal-pass-late-convergence-qualifications", runId)
const authRoot = path.join(root, ".runtime", "ai-painter", "owner-action-requests", "owner-authorized-stage4-general-late-convergence-qualification-20260813-074500000")
const smokeRoot = path.join(root, ".runtime", "ai-painter", "stage4-fact-conditioned-semantic-mixture-smoke-executions", "20260813-073000000")
const files = {
  authorization: path.join(authRoot, "implementation-authorization.json"),
  consumption: path.join(authRoot, "implementation-consumption.json"),
...
const evidence = Object.fromEntries(Object.entries(files).map(([key, file]) => [key, bind(file)]))
