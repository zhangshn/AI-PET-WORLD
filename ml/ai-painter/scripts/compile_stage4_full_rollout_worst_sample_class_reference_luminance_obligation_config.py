...
ALLOWED_ACTIONS = [
    "implement_stage4_full_rollout_worst_sample_class_reference_luminance_obligation_cpu_inactive_support",
...
def validate_authorization(
    ...
    if (
        ...
        != "owner-authorized-stage4-full-rollout-worst-sample-class-reference-luminance-cpu-implementation-v1"
        or authorization.get("status") != "resolved_owner_authorized_not_consumed"
        or not isinstance(request_id, str)
        or not request_id.startswith(
            "owner-authorized-stage4-full-rollout-worst-sample-class-reference-luminance-cpu-implementation-"
        )
        or authorization.get("commandRef") != request_id
        or authorization.get("scope")
        != "one_cpu_only_inactive_worst_sample_class_reference_luminance_obligation_implementation"
...
def compile_config(source: dict) -> dict:
    ...
    training[CONTRACT_KEY] = {
        ...
        "sourceContract": {
            ...
            "derivedWeights": deepcopy(source_contract["sourceContract"]["derivedWeights"]),
        ...
        "routeWestBoundaryNonRegression": {
        ...
        "evidenceBindings": deepcopy(
            trainer.STAGE4_FULL_ROLLOUT_WORST_SAMPLE_CLASS_REFERENCE_LUMINANCE_EVIDENCE_BINDINGS
        ),
...
def main() -> int:
    ...
    if source_path != configured_source or args.source_sha256 != authorization["sourceConfig"]["sha256"]:
        raise ValueError("inactive configuration source authorization changed")
    ...
    print(json.dumps({
        "status": "stage4_full_rollout_worst_sample_class_reference_luminance_inactive_config_compiled",
