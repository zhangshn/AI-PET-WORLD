...
def main() -> int:
    parser = ArgumentParser(description="Infer discrete-palette natural-home local detail models.")
    ...
    for category in NATURAL_CATEGORIES:
        rows.append(infer_category(category, args.dataset_root / category, args.model_root / category, args.output_root, torch, device))

    contact_sheet = build_contact_sheet(rows)
    contact_sheet_path = args.output_root / "contact-sheet.png"
    contact_sheet.save(contact_sheet_path)
    result = {
        "schemaVersion": "natural-home-local-detail-discrete-inference-v1",
        "status": "completed",
        "stageId": "natural-home-v1-no-building-local-details",
        "device": str(device),
        "contactSheet": str(contact_sheet_path.resolve()),
        "categories": rows,
        "note": "Discrete local pixel outputs are training diagnostics only. They cannot enter the formal world without VisualJudge and ApprovedFrame.",
    }
...
def infer_category(category: str, dataset_root: Path, model_root: Path, output_root: Path, torch, device) -> dict[str, object]:
    ...
    palette = np.asarray(json.loads((dataset_root / "palette.json").read_text(encoding="utf-8"))["colors"], dtype=np.uint8)
    ...
    with torch.inference_mode():
        class_map = model(condition)[0].argmax(dim=0).cpu().numpy()

    generated = Image.fromarray(palette[class_map])
...
def build_contact_sheet(rows: list[dict[str, object]]) -> Image.Image:
    ...
    for index, row in enumerate(rows):
        ...
        draw.text((x, y), f"{category}: target / discrete generated", fill="#dff8e6")
