...
CAPABILITY = "stage4-native-condition-encoder-clean-latent-change-candidate-v1"
TASK = "run_native_condition_encoder_readonly_gpu_qualification"
SEED = 20263722
...
def validate_ticket(ticket, ticket_path, consumption, output):
    ...
    expected_identity = {"seed": SEED, "imageSize": {"width": 256, "height": 192}, "sampleId": SAMPLE_ID, "split": "validation", "conditionChannels": 23, "latentChannels": 12}
...
def preflight(ticket, ticket_path, consumption, output):
    ...
    return {"schemaVersion": "stage4-native-condition-encoder-readonly-gpu-preflight-v1", "status": "passed_not_consumed", "cudaDevice": torch.cuda.get_device_name(0), "freeGpuMemoryBytes": int(free_gpu), "totalGpuMemoryBytes": int(total_gpu), "freeDiskBytes": int(free_disk), "recordedAtUtc": utc_now()}
...
def run_gpu(ticket, ticket_path, consumption, output):
    ...
    spatial = {}
    hooks = [
        model.denoiser.native_condition_stem.register_forward_hook(lambda _m, i, o: spatial.update({"stemInput": list(i[0].shape), "stemOutput": list(o.shape)})),
        model.denoiser.native_condition_down1.register_forward_hook(lambda _m, i, o: spatial.update({"down1Input": list(i[0].shape), "down1Output": list(o.shape)})),
        model.denoiser.native_condition_down2.register_forward_hook(lambda _m, i, o: spatial.update({"down2Input": list(i[0].shape), "down2Output": list(o.shape)})),
    ]
    ...
    predicted = model.predict_clean_latent(conditions)
    for hook in hooks:
        hook.remove()
    ...
    expected_spatial = {"stemInput": [1, 23, 192, 256], "stemOutput": [1, 64, 192, 256], "down1Input": [1, 64, 192, 256], "down1Output": [1, 128, 96, 128], "down2Input": [1, 128, 96, 128], "down2Output": [1, 256, 48, 64]}
    if spatial != expected_spatial:
        raise ValueError("native_spatial_identity_invalid")
    ...
    gradient = {"conditionGradientFiniteNonZero": True, "conditionGradientNorm": float(gradients[0].detach().norm().cpu()), "formalParameterTensorCount": len(parameters), "formalParameterCount": sum(parameter.numel() for parameter in parameters), "allFormalParameterGradientsFiniteNonZero": True, "cleanLatentLoss": float(clean_loss.detach().cpu()), "decodedRgbLoss": float(rgb_loss.detach().cpu()), "qualificationObjective": float(objective.detach().cpu()), "nativeSpatialIdentity": spatial}
    model.to("cpu")
    ...
    states = {"denoiserBefore": denoiser_before, "denoiserAfter": denoiser_after, "denoiserUnchanged": True, "autoencoderBefore": autoencoder_before, "autoencoderAfter": autoencoder_after, "autoencoderUnchanged": True}
    ...
    report = {"schemaVersion": "stage4-native-condition-encoder-readonly-gpu-report-v1", "status": "native_condition_encoder_readonly_gpu_qualification_succeeded", "capabilityVersion": CAPABILITY, "architecture": NATIVE_CONDITION_ENCODER_ARCHITECTURE, "sampleIdentity": {"sampleId": SAMPLE_ID, "split": "validation"}, "splitCounts": split_counts, "seed": SEED, "inputShape": [1, 23, 192, 256], "outputShape": [1, 12, 48, 64], "decodedRgbShape": [1, 3, 192, 256], "gradientEvidence": gradient, "stateHashes": states, "cuda": telemetry, "safety": {"denoiserCheckpointRead": False, "optimizerCreated": False, "backwardExecuted": False, "weightModified": False, "checkpointWritten": False, "smokeStarted": False, "trainingStarted": False}, "ticket": binding(ticket_path), "consumption": binding(consumption), "recordedAtUtc": utc_now()}
    write_atomic(output / "gpu-report.json", report)
    terminal = {"schemaVersion": "stage4-native-condition-encoder-readonly-gpu-terminal-v1", "executionState": "completed", "status": report["status"], "gpuReport": binding(output / "gpu-report.json"), "gradientEvidence": binding(output / "gradient-evidence.json"), "modelStateHashes": binding(output / "model-state-hashes.json"), "cudaTelemetry": binding(output / "cuda-telemetry.json"), "ownerAuthorizationRequired": False, "recordedAtUtc": utc_now()}
