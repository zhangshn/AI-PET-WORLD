...
CAPABILITY = "stage4-direct-clean-latent-responsibility-residual-change-candidate-v1"
TASK = "run_direct_responsibility_residual_readonly_gpu_qualification"
SEED = 20263722
...
def validate_ticket(ticket, ticket_path, consumption, output):
    ...
    for key in ("denoiserCheckpointRead", "optimizerCreation", "backwardExecution", "weightMutation", "checkpointWrite", "smoke", "training"):
        if permissions.get(key) is not False:
            raise ValueError(f"forbidden_permission_present:{key}")
    identity = ticket.get("executionIdentity", {})
    if identity != {"seed": SEED, "imageSize": {"width": 256, "height": 192}, "sampleId": SAMPLE_ID, "split": "validation", "conditionChannels": 23, "latentChannels": 12}:
...
def preflight(ticket, ticket_path, consumption, output):
    ...
    return {"schemaVersion": "stage4-direct-responsibility-residual-readonly-gpu-preflight-v1", "status": "passed_not_consumed", "cudaDevice": torch.cuda.get_device_name(0), "freeGpuMemoryBytes": int(free_gpu), "totalGpuMemoryBytes": int(total_gpu), "freeDiskBytes": int(free_disk), "recordedAtUtc": utc_now()}
...
def load_sample(config):
    ...
    for split in ("train", "validation", "challenge", "regression"):
        datasets[split] = AiAssistedConditionalDenoiserDataset(
            DATASET_PATH, split, list(config["conditionChannelOrder"]), (256, 192), selection_contract=selection
        )
...
def run_gpu(ticket, ticket_path, consumption, output):
    ...
    config = json.loads(resolve(Path(ticket["bindings"]["inactiveConfig"]["path"])).read_text(encoding="utf-8"))
    ...
    predicted, evidence = model.predict_clean_latent_with_responsibility_evidence(conditions)
    ...
    if tuple(evidence["responsibilityIdentityOrder"]) != RESPONSIBILITY_ORDER:
        raise ValueError("responsibility_order_invalid")

    isolation = []
    all_head_parameters = {
        name: tuple(model.denoiser.responsibility_residual_heads[name].parameters())
        for name in RESPONSIBILITY_ORDER
    }
    pointers = [parameter.data_ptr() for values in all_head_parameters.values() for parameter in values]
    if len(pointers) != len(set(pointers)):
        raise ValueError("responsibility_parameter_storage_shared")
    for identity, mask, residual in zip(RESPONSIBILITY_ORDER, evidence["responsibilityMasks"], evidence["maskedResponsibilityResiduals"]):
        outside = mask.expand_as(residual) == 0
        if bool(torch.any(residual[outside] != 0)):
            raise ValueError(f"outside_mask_residual_nonzero:{identity}")
        flattened = tuple(parameter for name in RESPONSIBILITY_ORDER for parameter in all_head_parameters[name])
        gradients = torch.autograd.grad(residual.square().mean(), flattened, retain_graph=True, allow_unused=True)
        offset = RESPONSIBILITY_ORDER.index(identity) * 2
        own = gradients[offset:offset + 2]
        other = gradients[:offset] + gradients[offset + 2:]
        if any(not finite_nonzero(value) for value in own):
            raise ValueError(f"own_head_gradient_invalid:{identity}")
        if any(value is not None and bool(torch.any(value != 0)) for value in other):
            raise ValueError(f"cross_head_gradient_leakage:{identity}")
        isolation.append({"identity": identity, "maskNonZeroCount": int(torch.count_nonzero(mask).detach().cpu()), "insideResidualFiniteNonZero": bool(torch.isfinite(residual[~outside]).all() and torch.any(residual[~outside] != 0)), "outsideResidualStrictZero": True, "ownParameterGradientFiniteNonZero": True, "otherHeadParameterGradientZeroOrUnused": True})
    ...
    gradients = torch.autograd.grad(objective, (conditions, *parameters), allow_unused=True)
    ...
    added_count = sum(parameter.numel() for parameter in model.denoiser.responsibility_residual_heads.parameters())
    if added_count != TOTAL_ADDED_PARAMETERS:
        raise ValueError("added_parameter_count_invalid")

    torch.cuda.synchronize(0)
    telemetry = {"deviceName": torch.cuda.get_device_name(0), "peakAllocatedBytes": int(torch.cuda.max_memory_allocated(0)), "peakReservedBytes": int(torch.cuda.max_memory_reserved(0)), "durationSeconds": round(time.perf_counter() - started, 3)}
    model.to("cpu")
    ...
    state = {"denoiserBefore": denoiser_before, "denoiserAfter": denoiser_after, "denoiserUnchanged": True, "autoencoderBefore": autoencoder_before, "autoencoderAfter": autoencoder_after, "autoencoderUnchanged": True}
    gradient = {"conditionGradientFiniteNonZero": True, "conditionGradientNorm": float(gradients[0].detach().norm().cpu()), "formalParameterTensorCount": len(parameters), "formalParameterCount": sum(parameter.numel() for parameter in parameters), "addedResponsibilityParameterCount": added_count, "allFormalParameterGradientsFiniteNonZero": True, "responsibilityIsolation": isolation, "cleanLatentLoss": float(clean_loss.detach().cpu()), "decodedRgbLoss": float(rgb_loss.detach().cpu()), "qualificationObjective": float(objective.detach().cpu())}
    write_atomic(output / "gradient-evidence.json", gradient)
    write_atomic(output / "model-state-hashes.json", state)
    write_atomic(output / "cuda-telemetry.json", {"status": "completed", **telemetry})
    report = {"schemaVersion": "stage4-direct-responsibility-residual-readonly-gpu-report-v1", "status": "direct_responsibility_residual_readonly_gpu_qualification_succeeded", "capabilityVersion": CAPABILITY, "architecture": DIRECT_RESPONSIBILITY_RESIDUAL_ARCHITECTURE, "sampleIdentity": {"sampleId": SAMPLE_ID, "split": "validation"}, "splitCounts": split_counts, "seed": SEED, "inputShape": [1, 23, 192, 256], "outputShape": [1, 12, 48, 64], "decodedRgbShape": [1, 3, 192, 256], "gradientEvidence": gradient, "stateHashes": state, "cuda": telemetry, "safety": {"denoiserCheckpointRead": False, "optimizerCreated": False, "backwardExecuted": False, "weightModified": False, "checkpointWritten": False, "smokeStarted": False, "trainingStarted": False}, "ticket": binding(ticket_path), "consumption": binding(consumption), "recordedAtUtc": utc_now()}
    write_atomic(output / "gpu-report.json", report)
    terminal = {"schemaVersion": "stage4-direct-responsibility-residual-readonly-gpu-terminal-v1", "executionState": "completed", "status": report["status"], "gpuReport": binding(output / "gpu-report.json"), "gradientEvidence": binding(output / "gradient-evidence.json"), "modelStateHashes": binding(output / "model-state-hashes.json"), "cudaTelemetry": binding(output / "cuda-telemetry.json"), "ownerAuthorizationRequired": False, "recordedAtUtc": utc_now()}
...
def main():
    ...
    if args.preflight_only:
        ...
        return 0
    write_exclusive(consumption, {"schemaVersion": "ai-painter-local-internal-readonly-gpu-ticket-consumption-v1", "status": "atomically_consumed", "ticket": binding(ticket_path), "capabilityVersion": CAPABILITY, "taskId": TASK, "oneTimeConsumption": True, "consumedAtUtc": utc_now()})
