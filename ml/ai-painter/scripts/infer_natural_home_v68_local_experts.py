...
def main() -> int:
    parser = ArgumentParser(description="Infer MVP natural-home local experts and build target/generated review sheet.")
    ...
    rows = [
        infer_category(category, args.dataset_root / category, args.model_root / category, args.output_root, torch, device)
        for category in MVP_LOCAL_EXPERTS
    ]
    contact_sheet_path = args.output_root / "contact-sheet.png"
    build_contact_sheet(rows).save(contact_sheet_path)
    result = {
        "schemaVersion": f"natural-home-{args.stage}-local-expert-inference-v1",
        "status": "completed",
        "stage": args.stage,
