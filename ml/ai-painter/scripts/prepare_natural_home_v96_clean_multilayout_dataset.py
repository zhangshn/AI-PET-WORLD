...
def main() -> int:
    parser = ArgumentParser(description="Prepare V96 clean multi-layout natural-home dataset.")
    ...
    parser.add_argument("--quality-report", type=Path, default=Path(".runtime/ai-painter/natural-home-v95-failure-repair-quality-selection/latest.json"))
    parser.add_argument("--vj1-report", type=Path, default=Path(".runtime/ai-painter/natural-home-v95-failure-repair-vj1-review/latest.json"))
    parser.add_argument("--output-root", type=Path, default=Path(".runtime/ai-painter/natural-home-v96-clean-multilayout-dataset"))
    parser.add_argument("--stage-id", default="natural-home-v96-clean-multilayout-dataset")
    parser.add_argument("--training-target-source", default="v96_clean_multilayout_same_source_target_png")
    parser.add_argument("--max-focus-sources", type=int, default=24)
    ...
    if args.output_root.exists():
        ...
        preserve_runtime_dir_before_clear(args.output_root, "prepare-natural-home-v96-clean-multilayout-dataset")
    ...
    for focus_source_id in focus_sources:
        ...
        for index in range(1, args.focus_repeat + 1):
            copy_id = f"{focus_source_id}__v96focus{index:02d}"
            copy_training_sample(
                source_scene_root,
                output_scene_root,
                focus_source_id,
                ...
                args.stage_id,
                args.training_target_source,
                focus_record={**focus_record, "copyIndex": index, "copyCount": args.focus_repeat},
            )
            focus_copy_ids.append(copy_id)
        focus_records.append({**focus_record, "copyCount": args.focus_repeat})
    ...
    manifest = {
        "schemaVersion": "natural-home-v96-clean-multilayout-dataset-v1",
        "status": "completed",
        ...
        "policy": {
            ...
            "displayAllowed": False,
            "canPromoteToWorld": False,
            "purpose": "Train clean multi-layout natural home expression without buildings, characters, animals, or dynamic content.",
        },
    }
    write_json(args.output_root / "manifest.json", manifest)
    write_json(args.output_root / "latest.json", manifest)
    write_json(args.output_root / "focus-records.json", {
        "schemaVersion": "natural-home-v96-focus-records-v1",
        "stageId": args.stage_id,
        "focusSourceCount": len(focus_sources),
        "focusCopySampleCount": len(focus_copy_ids),
        "rows": focus_records,
...
def choose_focus_sources(
    ...
    for sample_id in train_ids:
        ...
        severity = len(reasons) * 8.0 + (12.0 if quality_status == "rejected_training_candidate" else 0.0) + max(0.0, 92.0 - score)
...
def copy_training_sample(
    ...
    *,
    ...
    if focus_record:
        metadata["v96CleanMultilayoutFocus"] = focus_record
...
def source_id(sample_id: str) -> str:
    return sample_id.split("__v28-", 1)[0].split("__v96focus", 1)[0]


def variant_id(sample_id: str) -> str:
    if "__v28-" not in sample_id:
        return "source"
    return sample_id.split("__v28-", 1)[1].split("__v96focus", 1)[0]
