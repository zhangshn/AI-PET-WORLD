...
def expect_failure(name: str, mutator, inactive: dict, contract: dict) -> dict:
    candidate = compile_direct_responsibility_residual_stage0_active_config(
        inactive, contract, ticket_state="consumed"
    )
    mutator(candidate)
    try:
        validate_direct_responsibility_residual_stage0_active_config(candidate, contract)
...
def main() -> int:
    ...
    preflight = compile_direct_responsibility_residual_stage0_active_config(
        inactive, contract, ticket_state="preflight_unconsumed"
    )
    consumed = compile_direct_responsibility_residual_stage0_active_config(
        inactive, contract, ticket_state="consumed"
    )
    positive = [
        {"name": "preflight_contract", **validate_direct_responsibility_residual_stage0_active_config(preflight, contract)},
        {"name": "consumed_contract", **validate_direct_responsibility_residual_stage0_active_config(consumed, contract)},
        {
            "name": "formal_identity",
            "status": "passed",
            "splitCounts": consumed["training"]["directResponsibilityResidualFormalStage0"]["splitCounts"],
            "previewEpochs": consumed["training"]["directResponsibilityResidualFormalStage0"]["previewEpochs"],
            "activeGates": sorted(
    ...
    negative = [
        ...
        expect_failure("wrong_split", lambda value: value["training"]["directResponsibilityResidualFormalStage0"]["splitCounts"].__setitem__("train", 47), inactive, contract),
        ...
        expect_failure("automatic_review_disabled", lambda value: value["training"]["directResponsibilityResidualFormalStage0"].__setitem__("automaticReviewAfterTraining", False), inactive, contract),
        expect_failure("owner_gate_reintroduced", lambda value: value.__setitem__("ownerAuthorizationRequired", True), inactive, contract),
        expect_failure("historical_checkpoint_allowed", lambda value: value["directResponsibilityResidualContract"].__setitem__("historicalCheckpointAllowed", True), inactive, contract),
    ]
    report = {
        "schemaVersion": "stage4-direct-responsibility-residual-stage0-cpu-report-v1",
        "status": "stage4_direct_responsibility_residual_stage0_cpu_preflight_passed",
